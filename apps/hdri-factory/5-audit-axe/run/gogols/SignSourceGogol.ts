...
<keywords>sign, signature, ed25519, content-hash, traceability, axe.db</keywords>
<responsibilities>
  <item>Computes SHA-256 hash of the final axe.db file.</item>
...
<MODULE_MAP>
  <entry key="SignSourceGogol">Signs axe.db file hash for downstream verification.</entry>
</MODULE_MAP>
<CHANGE_SUMMARY>
...
export class SignSourceGogol extends Gogol {
  ...
  override readonly guide = {
    title: 'Sign source',
    purpose: 'Create cryptographic signature over axe.db file hash for downstream verification and traceability.',
  ...
  override async run(ctx: PipelineContext): Promise<void> {
    ...
    const axeDbPath = getAuditsDbPath(year);

    const signingKey = loadSigningKeyFromEnv();

    const contentHash = await hashDatabaseFile(axeDbPath);
    ...
    console.log(`[sign-source] axe.db signed. hash=${contentHash} key=${manifest.signing_key_id}`);
  }
}
