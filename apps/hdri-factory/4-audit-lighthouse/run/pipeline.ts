/*
<MODULE_CONTRACT>
<purpose>Facilitates the construction and execution of the lighthouse audit pipeline.</purpose>
<keywords>pipeline, lighthouse, audit</keywords>
<responsibilities>
...
const createExecutionSummary = (): string =>
  [
    'Site deep-audit (T3): reads the scoring cohort, sub-samples deterministically,',
    'runs Lighthouse per site through a rate-limited executor, stores',
    'raw JSON reports in CAS on disk, and persists per-tool metrics to lighthouse_YYYY.db.',
  ].join(' ');
