...
<keywords>sign, signature, ed25519, content-hash, traceability, lighthouse.db</keywords>
<responsibilities>
  <item>Computes SHA-256 hash of the final lighthouse.db file.</item>
...
<MODULE_MAP>
  <entry key="SignSourceGogol">Signs lighthouse.db file hash for downstream verification.</entry>
</MODULE_MAP>
<CHANGE_SUMMARY>
...
export class SignSourceGogol extends Gogol {
  ...
  override readonly guide = {
    title: 'Sign source',
    purpose: 'Create cryptographic signature over lighthouse.db file hash for downstream verification and traceability.',
  ...
  override async run(ctx: PipelineContext): Promise<void> {
    ...
    const lighthouseDbPath = getAuditsDbPath(year);

    const signingKey = loadSigningKeyFromEnv();

    const contentHash = await hashDatabaseFile(lighthouseDbPath);
    ...
    console.log(`[sign-source] lighthouse.db signed. hash=${contentHash} key=${manifest.signing_key_id}`);
  }
}
