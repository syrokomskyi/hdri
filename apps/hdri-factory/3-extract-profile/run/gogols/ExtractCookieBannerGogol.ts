...
<keywords>extract, cookie banner, consent, GDPR, ext_cookie_banner</keywords>
<responsibilities>
  ...
  <item>Call extractCookieBanner() and upsert one row per content_sha256 into ext_cookie_banner.</item>
...
<MODULE_MAP>
  <entry key="ExtractCookieBannerGogol.run">Main extraction loop over page_observations for the current batch.</entry>
</MODULE_MAP>
<CHANGE_SUMMARY>
...
import { extractCookieBanner } from '@org/business-crawler/extract';
...
export class ExtractCookieBannerGogol extends ExtractGogolBase {
  ...
  protected override get csvColumns(): string[] { return ['content_sha256', 'present', 'confidence', 'quality']; }
