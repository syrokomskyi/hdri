/*
<MODULE_CONTRACT>
<purpose>Detects certifications (DIN, ISO, TÜV, DEKRA, etc.) on crawled pages and writes results to ext_certifications.</purpose>
<keywords>extract, certifications, Zertifikat, ISO, DIN, TÜV, ext_certifications</keywords>
<responsibilities>
  ...
  <item>Call extractCertifications() and upsert one row per content_sha256 into ext_certifications.</item>
  <item>Write extract-report.json artifact with counts.</item>
</responsibilities>
<non-goals>
  <item>Do not fetch pages — that is CrawlGogol's responsibility.</item>
  <item>Do not write to any table other than ext_certifications.</item>
</non-goals>
</MODULE_CONTRACT>
<MODULE_MAP>
  <entry key="ExtractCertificationsGogol.run">Main extraction loop over page_observations for the current batch.</entry>
</MODULE_MAP>
<CHANGE_SUMMARY>
...
import { extractCertifications } from '@org/business-crawler/extract';
import { ExtractGogolBase, type ObsRow } from './ExtractGogolBase.js';

export class ExtractCertificationsGogol extends ExtractGogolBase {
  override readonly id = 'extract-certifications';
  override readonly table = 'ext_certifications';

  protected override extractDom($: CheerioAPI, _row: ObsRow): unknown[] | null {
    const r = extractCertifications($);
  ...
  protected override get upsertSql(): string {
    return `INSERT INTO ext_certifications (content_sha256, extractor_ver, present, confidence) VALUES (?, ?, ?, ?)
