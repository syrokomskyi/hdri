/*
<MODULE_CONTRACT>
<purpose>Detects embedded maps (Google Maps, OpenStreetMap, etc.) on crawled pages and writes results to ext_map.</purpose>
<keywords>extract, map, Google Maps, OpenStreetMap, iframe, ext_map</keywords>
<responsibilities>
  ...
  <item>Call extractMap() and upsert one row per content_sha256 into ext_map.</item>
  <item>Write extract-report.json artifact with counts.</item>
</responsibilities>
<non-goals>
  <item>Do not fetch pages — that is CrawlGogol's responsibility.</item>
  <item>Do not write to any table other than ext_map.</item>
</non-goals>
</MODULE_CONTRACT>
<MODULE_MAP>
  <entry key="ExtractMapGogol.run">Main extraction loop over page_observations for the current batch.</entry>
</MODULE_MAP>
<CHANGE_SUMMARY>
...
import { extractMap } from '@org/business-crawler/extract';
import { ExtractGogolBase, type ObsRow } from './ExtractGogolBase.js';

export class ExtractMapGogol extends ExtractGogolBase {
  override readonly id = 'extract-map';
  override readonly table = 'ext_map';
  ...
  protected override get upsertSql(): string {
    return `INSERT INTO ext_map (content_sha256, extractor_ver, present, confidence) VALUES (?, ?, ?, ?)
