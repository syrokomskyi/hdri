/*
<MODULE_CONTRACT>
<purpose>Detects Instagram profile links on crawled pages and writes results to ext_social_instagram.</purpose>
<keywords>extract, social, Instagram, ext_social_instagram</keywords>
<responsibilities>
  ...
  <item>Call extractSocialInstagram() and upsert one row per content_sha256 into ext_social_instagram.</item>
  <item>Write extract-report.json artifact with counts.</item>
</responsibilities>
<non-goals>
  <item>Do not fetch pages — that is CrawlGogol's responsibility.</item>
  <item>Do not write to any table other than ext_social_instagram.</item>
</non-goals>
</MODULE_CONTRACT>
<MODULE_MAP>
  <entry key="ExtractSocialInstagramGogol.run">Main extraction loop over page_observations for the current batch.</entry>
</MODULE_MAP>
<CHANGE_SUMMARY>
...
import { extractSocialInstagram } from '@org/business-crawler/extract';
import { ExtractGogolBase, type ObsRow } from './ExtractGogolBase.js';

export class ExtractSocialInstagramGogol extends ExtractGogolBase {
  override readonly id = 'extract-social-instagram';
  override readonly table = 'ext_social_instagram';

  protected override extractDom($: CheerioAPI, _row: ObsRow): unknown[] | null {
    const r = extractSocialInstagram($);
  ...
  protected override get upsertSql(): string {
    return `INSERT INTO ext_social_instagram (content_sha256, extractor_ver, present, url) VALUES (?, ?, ?, ?)
