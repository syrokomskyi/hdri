/*
<MODULE_CONTRACT>
<purpose>Detects links to Google Business profiles on crawled pages and writes results to ext_link_google_business.</purpose>
<keywords>extract, Google Business, g.page, maps.app.goo.gl, ext_link_google_business</keywords>
<responsibilities>
  ...
  <item>Call extractLinkGoogleBusiness() and upsert one row per content_sha256 into ext_link_google_business.</item>
...
<non-goals>
  ...
  <item>Do not write to any table other than ext_link_google_business.</item>
</non-goals>
</MODULE_CONTRACT>
<MODULE_MAP>
  <entry key="ExtractLinkGoogleBusinessGogol.run">Main extraction loop over page_observations for the current batch.</entry>
</MODULE_MAP>
<CHANGE_SUMMARY>
...
import { extractLinkGoogleBusiness } from '@org/business-crawler/extract';
import { ExtractGogolBase, type ObsRow } from './ExtractGogolBase.js';

export class ExtractLinkGoogleBusinessGogol extends ExtractGogolBase {
  override readonly id = 'extract-link-google-business';
  override readonly table = 'ext_link_google_business';

  protected override extractDom($: CheerioAPI, _row: ObsRow): unknown[] | null {
    const r = extractLinkGoogleBusiness($);
  ...
  protected override get upsertSql(): string {
    return `INSERT INTO ext_link_google_business (content_sha256, extractor_ver, present, url) VALUES (?, ?, ?, ?)
