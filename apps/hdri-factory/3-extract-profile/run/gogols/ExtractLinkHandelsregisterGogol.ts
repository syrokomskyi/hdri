/*
<MODULE_CONTRACT>
<purpose>Detects links to Handelsregister on crawled pages and writes results to ext_link_handelsregister.</purpose>
<keywords>extract, Handelsregister, external link, ext_link_handelsregister</keywords>
<responsibilities>
  ...
  <item>Call extractLinkHandelsregister() and upsert one row per content_sha256 into ext_link_handelsregister.</item>
...
<non-goals>
  ...
  <item>Do not write to any table other than ext_link_handelsregister.</item>
</non-goals>
</MODULE_CONTRACT>
<MODULE_MAP>
  <entry key="ExtractLinkHandelsregisterGogol.run">Main extraction loop over page_observations for the current batch.</entry>
</MODULE_MAP>
<CHANGE_SUMMARY>
...
import { extractLinkHandelsregister } from '@org/business-crawler/extract';
import { ExtractGogolBase, type ObsRow } from './ExtractGogolBase.js';

export class ExtractLinkHandelsregisterGogol extends ExtractGogolBase {
  override readonly id = 'extract-link-handelsregister';
  override readonly table = 'ext_link_handelsregister';

  protected override extractDom($: CheerioAPI, _row: ObsRow): unknown[] | null {
    const r = extractLinkHandelsregister($);
  ...
  protected override get upsertSql(): string {
    return `INSERT INTO ext_link_handelsregister (content_sha256, extractor_ver, present, url) VALUES (?, ?, ?, ?)
