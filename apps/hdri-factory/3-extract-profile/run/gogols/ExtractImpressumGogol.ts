/*
<MODULE_CONTRACT>
<purpose>Extracts impressum (legal notice) signal from crawled pages and writes results to ext_impressum.</purpose>
<keywords>extract, impressum, compliance, DSGVO, ext_impressum</keywords>
<responsibilities>
  ...
  <item>Call extractImpressum() and upsert one row per content_sha256 into ext_impressum.</item>
...
<non-goals>
  ...
  <item>Do not write to any table other than ext_impressum.</item>
</non-goals>
</MODULE_CONTRACT>
<MODULE_MAP>
  <entry key="ExtractImpressumGogol.run">Main extraction loop over page_observations for the current batch.</entry>
</MODULE_MAP>
<CHANGE_SUMMARY>
...
import { extractImpressum } from '@org/business-crawler/extract';
import { ExtractGogolBase, type ObsRow } from './ExtractGogolBase.js';

export class ExtractImpressumGogol extends ExtractGogolBase {
  override readonly id = 'extract-impressum';
  override readonly table = 'ext_impressum';
  ...
  protected override extractDom($: CheerioAPI, row: ObsRow): unknown[] | null {
    const r = extractImpressum($, row.url_norm!);
  ...
  protected override get upsertSql(): string {
    return `INSERT INTO ext_impressum (content_sha256, extractor_ver, present, url, confidence) VALUES (?, ?, ?, ?, ?)
