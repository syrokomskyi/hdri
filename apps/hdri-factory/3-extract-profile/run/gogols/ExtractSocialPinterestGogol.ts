/*
<MODULE_CONTRACT>
<purpose>Detects Pinterest profile links on crawled pages and writes results to ext_social_pinterest.</purpose>
<keywords>extract, social, Pinterest, ext_social_pinterest</keywords>
<responsibilities>
  ...
  <item>Call extractSocialPinterest() and upsert one row per content_sha256 into ext_social_pinterest.</item>
  <item>Write extract-report.json artifact with counts.</item>
</responsibilities>
<non-goals>
  <item>Do not fetch pages — that is CrawlGogol's responsibility.</item>
  <item>Do not write to any table other than ext_social_pinterest.</item>
</non-goals>
</MODULE_CONTRACT>
<MODULE_MAP>
  <entry key="ExtractSocialPinterestGogol.run">Main extraction loop over page_observations for the current batch.</entry>
</MODULE_MAP>
<CHANGE_SUMMARY>
...
import { extractSocialPinterest } from '@org/business-crawler/extract';
import { ExtractGogolBase, type ObsRow } from './ExtractGogolBase.js';

export class ExtractSocialPinterestGogol extends ExtractGogolBase {
  override readonly id = 'extract-social-pinterest';
  override readonly table = 'ext_social_pinterest';

  protected override extractDom($: CheerioAPI, _row: ObsRow): unknown[] | null {
    const r = extractSocialPinterest($);
  ...
  protected override get upsertSql(): string {
    return `INSERT INTO ext_social_pinterest (content_sha256, extractor_ver, present, url) VALUES (?, ?, ?, ?)
