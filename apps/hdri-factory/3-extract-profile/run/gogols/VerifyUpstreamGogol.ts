/*
<MODULE_CONTRACT>
<purpose>Verifies upstream 2-check-liveness signatures before consuming liveness.db files.</purpose>
<keywords>verify, upstream, signature, ed25519, liveness.db, transparency</keywords>
<responsibilities>
  <item>Loads public keys from transparency/keys/ directory.</item>
  <item>Discovers upstream liveness.db files and their source-signature.json manifests.</item>
  <item>Verifies ed25519 signatures against the corresponding public keys.</item>
  <item>Re-computes SHA-256 of each liveness.db and compares it to the signed content hash.</item>
...
export class VerifyUpstreamGogol extends Gogol {
  ...
  override readonly guide = {
    ...
    purpose: 'Check ed25519 signatures on every upstream 2-check-liveness liveness.db before ingestion.',
  ...
  override async run(ctx: PipelineContext): Promise<void> {
    ...
    const livenessDbName = `liveness_${year}.db`;
    ...
    const devices = await listDeviceFolders(upstreamLivenessOutputRoot);
    const entries: VerificationEntry[] = [];
    let allOk = true;

    for (const dev of devices) {
      const dbPath = path.join(dev.path, 'data', 'db', livenessDbName);
      if (!fs.existsSync(dbPath)) {
        continue;
      }

      // Search for source-signature.json under the device output tree
      const manifestPath = await findManifestPath(dev.path, { expectedAppId: '2-check-liveness' });
      if (!manifestPath) {
        allOk = false;
        entries.push({
      ...
      // Re-compute SHA-256 of liveness.db and compare
    ...
    await reporter.writeSummary({
      ...
      year,
      upstreamRoot: upstreamLivenessOutputRoot,
      transparencyDir,
      allOk,
      entries,
