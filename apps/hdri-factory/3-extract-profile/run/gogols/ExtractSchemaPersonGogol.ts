/*
<MODULE_CONTRACT>
<purpose>Detects Schema.org Person markup in crawled pages and writes results to ext_schema_person.</purpose>
<keywords>extract, schema.org, Person, JSON-LD, ext_schema_person</keywords>
<responsibilities>
  ...
  <item>Call extractSchemaPerson() and upsert one row per content_sha256 into ext_schema_person.</item>
  <item>Write extract-report.json artifact with counts.</item>
</responsibilities>
<non-goals>
  <item>Do not fetch pages — that is CrawlGogol's responsibility.</item>
  <item>Do not write to any table other than ext_schema_person.</item>
</non-goals>
</MODULE_CONTRACT>
<MODULE_MAP>
  <entry key="ExtractSchemaPersonGogol.run">Main extraction loop over page_observations for the current batch.</entry>
</MODULE_MAP>
<CHANGE_SUMMARY>
...
import { extractSchemaPerson } from '@org/business-crawler/extract';
import { ExtractGogolBase, type ObsRow } from './ExtractGogolBase.js';

export class ExtractSchemaPersonGogol extends ExtractGogolBase {
  override readonly id = 'extract-schema-person';
  override readonly table = 'ext_schema_person';

  protected override extractDom($: CheerioAPI, _row: ObsRow): unknown[] | null {
    return [extractSchemaPerson($).present ? 1 : 0];
  }
  protected override get csvColumns(): string[] { return ['content_sha256', 'present']; }
  protected override get upsertSql(): string {
    return `INSERT INTO ext_schema_person (content_sha256, extractor_ver, present) VALUES (?, ?, ?)
