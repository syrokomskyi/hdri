/*
<MODULE_CONTRACT>
<purpose>Detects Schema.org HowTo markup in crawled pages and writes results to ext_schema_how_to.</purpose>
<keywords>extract, schema.org, HowTo, JSON-LD, ext_schema_how_to</keywords>
<responsibilities>
  ...
  <item>Call extractSchemaHowTo() and upsert one row per content_sha256 into ext_schema_how_to.</item>
  <item>Write extract-report.json artifact with counts.</item>
</responsibilities>
<non-goals>
  <item>Do not fetch pages — that is CrawlGogol's responsibility.</item>
  <item>Do not write to any table other than ext_schema_how_to.</item>
</non-goals>
</MODULE_CONTRACT>
<MODULE_MAP>
  <entry key="ExtractSchemaHowToGogol.run">Main extraction loop over page_observations for the current batch.</entry>
</MODULE_MAP>
<CHANGE_SUMMARY>
...
import { extractSchemaHowTo } from '@org/business-crawler/extract';
import { ExtractGogolBase, type ObsRow } from './ExtractGogolBase.js';

export class ExtractSchemaHowToGogol extends ExtractGogolBase {
  override readonly id = 'extract-schema-how-to';
  override readonly table = 'ext_schema_how_to';

  protected override extractDom($: CheerioAPI, _row: ObsRow): unknown[] | null {
    return [extractSchemaHowTo($).present ? 1 : 0];
  }
  protected override get csvColumns(): string[] { return ['content_sha256', 'present']; }
  protected override get upsertSql(): string {
    return `INSERT INTO ext_schema_how_to (content_sha256, extractor_ver, present) VALUES (?, ?, ?)
