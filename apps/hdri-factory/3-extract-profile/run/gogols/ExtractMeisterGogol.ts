/*
<MODULE_CONTRACT>
<purpose>Detects Meister/Handwerksmeister mentions on crawled pages and writes results to ext_meister.</purpose>
<keywords>extract, Meister, Meisterbetrieb, Handwerksmeister, HWK, ext_meister</keywords>
<responsibilities>
  ...
  <item>Call extractMeister() and upsert one row per content_sha256 into ext_meister.</item>
  <item>Write extract-report.json artifact with counts.</item>
</responsibilities>
<non-goals>
  <item>Do not fetch pages — that is CrawlGogol's responsibility.</item>
  <item>Do not write to any table other than ext_meister.</item>
</non-goals>
</MODULE_CONTRACT>
<MODULE_MAP>
  <entry key="ExtractMeisterGogol.run">Main extraction loop over page_observations for the current batch.</entry>
</MODULE_MAP>
<CHANGE_SUMMARY>
...
import { extractMeister } from '@org/business-crawler/extract';
import { ExtractGogolBase, type ObsRow } from './ExtractGogolBase.js';

export class ExtractMeisterGogol extends ExtractGogolBase {
  override readonly id = 'extract-meister';
  override readonly table = 'ext_meister';

  protected override extractDom($: CheerioAPI, _row: ObsRow): unknown[] | null {
    const r = extractMeister($);
  ...
  protected override get upsertSql(): string {
    return `INSERT INTO ext_meister (content_sha256, extractor_ver, present, confidence) VALUES (?, ?, ?, ?)
