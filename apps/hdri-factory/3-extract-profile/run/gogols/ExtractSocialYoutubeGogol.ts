/*
<MODULE_CONTRACT>
<purpose>Detects YouTube channel links on crawled pages and writes results to ext_social_youtube.</purpose>
<keywords>extract, social, YouTube, youtu.be, ext_social_youtube</keywords>
<responsibilities>
  ...
  <item>Call extractSocialYoutube() and upsert one row per content_sha256 into ext_social_youtube.</item>
  <item>Write extract-report.json artifact with counts.</item>
</responsibilities>
<non-goals>
  <item>Do not fetch pages — that is CrawlGogol's responsibility.</item>
  <item>Do not write to any table other than ext_social_youtube.</item>
</non-goals>
</MODULE_CONTRACT>
<MODULE_MAP>
  <entry key="ExtractSocialYoutubeGogol.run">Main extraction loop over page_observations for the current batch.</entry>
</MODULE_MAP>
<CHANGE_SUMMARY>
...
import { extractSocialYoutube } from '@org/business-crawler/extract';
import { ExtractGogolBase, type ObsRow } from './ExtractGogolBase.js';

export class ExtractSocialYoutubeGogol extends ExtractGogolBase {
  override readonly id = 'extract-social-youtube';
  override readonly table = 'ext_social_youtube';

  protected override extractDom($: CheerioAPI, _row: ObsRow): unknown[] | null {
    const r = extractSocialYoutube($);
  ...
  protected override get upsertSql(): string {
    return `INSERT INTO ext_social_youtube (content_sha256, extractor_ver, present, url) VALUES (?, ?, ?, ?)
