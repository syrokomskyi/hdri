/*
<MODULE_CONTRACT>
<purpose>Detects contact forms on crawled pages and writes results to ext_contact_form.</purpose>
<keywords>extract, contact form, Kontaktformular, ext_contact_form</keywords>
<responsibilities>
  ...
  <item>Call extractContactForm() and upsert one row per content_sha256 into ext_contact_form.</item>
  <item>Write extract-report.json artifact with counts.</item>
</responsibilities>
<non-goals>
  <item>Do not fetch pages — that is CrawlGogol's responsibility.</item>
  <item>Do not write to any table other than ext_contact_form.</item>
</non-goals>
</MODULE_CONTRACT>
<MODULE_MAP>
  <entry key="ExtractContactFormGogol.run">Main extraction loop over page_observations for the current batch.</entry>
</MODULE_MAP>
<CHANGE_SUMMARY>
...
import { extractContactForm } from '@org/business-crawler/extract';
import { ExtractGogolBase, type ObsRow } from './ExtractGogolBase.js';

export class ExtractContactFormGogol extends ExtractGogolBase {
  override readonly id = 'extract-contact-form';
  override readonly table = 'ext_contact_form';

  protected override extractDom($: CheerioAPI, _row: ObsRow): unknown[] | null {
    const r = extractContactForm($);
  ...
  protected override get upsertSql(): string {
    return `INSERT INTO ext_contact_form (content_sha256, extractor_ver, present, confidence) VALUES (?, ?, ?, ?)
