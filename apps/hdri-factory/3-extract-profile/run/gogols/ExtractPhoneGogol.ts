/*
<MODULE_CONTRACT>
<purpose>Extracts phone numbers from crawled pages and writes results to ext_phone.</purpose>
<keywords>extract, phone, Telefon, ext_phone</keywords>
<responsibilities>
  ...
  <item>Call extractPageSignals() and extract phone counts, upsert one row per content_sha256 into ext_phone.</item>
...
<non-goals>
  ...
  <item>Does not write to any table other than ext_phone.</item>
</non-goals>
</MODULE_CONTRACT>
<MODULE_MAP>
  <entry key="ExtractPhoneGogol.run">Main extraction loop over page_observations for the current batch.</entry>
</MODULE_MAP>
<CHANGE_SUMMARY>
  <item>Initial implementation: extract phone numbers from HTML and store count in ext_phone.</item>
...
export class ExtractPhoneGogol extends ExtractGogolBase {
