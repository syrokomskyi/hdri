/*
<MODULE_CONTRACT>
<purpose>Detects awards and prizes mentions on crawled pages and writes results to ext_awards.</purpose>
<keywords>extract, awards, Auszeichnung, Preis, ext_awards</keywords>
<responsibilities>
  ...
  <item>Call extractAwards() and upsert one row per content_sha256 into ext_awards.</item>
  <item>Write extract-report.json artifact with counts.</item>
</responsibilities>
<non-goals>
  <item>Do not fetch pages — that is CrawlGogol's responsibility.</item>
  <item>Do not write to any table other than ext_awards.</item>
</non-goals>
</MODULE_CONTRACT>
<MODULE_MAP>
  <entry key="ExtractAwardsGogol.run">Main extraction loop over page_observations for the current batch.</entry>
</MODULE_MAP>
<CHANGE_SUMMARY>
...
import { extractAwards } from '@org/business-crawler/extract';
import { ExtractGogolBase, type ObsRow } from './ExtractGogolBase.js';

export class ExtractAwardsGogol extends ExtractGogolBase {
  override readonly id = 'extract-awards';
  override readonly table = 'ext_awards';

  protected override extractDom($: CheerioAPI, _row: ObsRow): unknown[] | null {
    const r = extractAwards($);
  ...
  protected override get upsertSql(): string {
    return `INSERT INTO ext_awards (content_sha256, extractor_ver, present, confidence) VALUES (?, ?, ?, ?)
