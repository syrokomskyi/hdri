/*
<MODULE_CONTRACT>
<purpose>Detects Widerruf (right-of-withdrawal) links on crawled pages and writes results to ext_widerruf_page.</purpose>
<keywords>extract, Widerruf, right-of-withdrawal, legal, ext_widerruf_page</keywords>
<responsibilities>
  ...
  <item>Call extractWiderrufPage() and upsert one row per content_sha256 into ext_widerruf_page.</item>
  <item>Write extract-report.json artifact with counts.</item>
</responsibilities>
<non-goals>
  <item>Do not fetch pages — that is CrawlGogol's responsibility.</item>
  <item>Do not write to any table other than ext_widerruf_page.</item>
</non-goals>
</MODULE_CONTRACT>
<MODULE_MAP>
  <entry key="ExtractWiderrufPageGogol.run">Main extraction loop over page_observations for the current batch.</entry>
</MODULE_MAP>
<CHANGE_SUMMARY>
...
import { extractWiderrufPage } from '@org/business-crawler/extract';
import { ExtractGogolBase, type ObsRow } from './ExtractGogolBase.js';

export class ExtractWiderrufPageGogol extends ExtractGogolBase {
  override readonly id = 'extract-widerruf-page';
  override readonly table = 'ext_widerruf_page';
  ...
  protected override extractDom($: CheerioAPI, row: ObsRow): unknown[] | null {
    const r = extractWiderrufPage($, row.url_norm!);
  ...
  protected override get upsertSql(): string {
    return `INSERT INTO ext_widerruf_page (content_sha256, extractor_ver, present, url, confidence) VALUES (?, ?, ?, ?, ?)
