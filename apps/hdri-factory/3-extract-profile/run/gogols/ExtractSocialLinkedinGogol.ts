/*
<MODULE_CONTRACT>
<purpose>Detects LinkedIn profile links on crawled pages and writes results to ext_social_linkedin.</purpose>
<keywords>extract, social, LinkedIn, ext_social_linkedin</keywords>
<responsibilities>
  ...
  <item>Call extractSocialLinkedin() and upsert one row per content_sha256 into ext_social_linkedin.</item>
  <item>Write extract-report.json artifact with counts.</item>
</responsibilities>
<non-goals>
  <item>Do not fetch pages — that is CrawlGogol's responsibility.</item>
  <item>Do not write to any table other than ext_social_linkedin.</item>
</non-goals>
</MODULE_CONTRACT>
<MODULE_MAP>
  <entry key="ExtractSocialLinkedinGogol.run">Main extraction loop over page_observations for the current batch.</entry>
</MODULE_MAP>
<CHANGE_SUMMARY>
...
import { extractSocialLinkedin } from '@org/business-crawler/extract';
import { ExtractGogolBase, type ObsRow } from './ExtractGogolBase.js';

export class ExtractSocialLinkedinGogol extends ExtractGogolBase {
  override readonly id = 'extract-social-linkedin';
  override readonly table = 'ext_social_linkedin';

  protected override extractDom($: CheerioAPI, _row: ObsRow): unknown[] | null {
    const r = extractSocialLinkedin($);
  ...
  protected override get upsertSql(): string {
    return `INSERT INTO ext_social_linkedin (content_sha256, extractor_ver, present, url) VALUES (?, ?, ?, ?)
