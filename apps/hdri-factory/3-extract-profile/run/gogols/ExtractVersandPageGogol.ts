/*
<MODULE_CONTRACT>
<purpose>Detects Versand/Lieferung (shipping) page links on crawled pages and writes results to ext_versand_page.</purpose>
<keywords>extract, Versand, shipping, delivery, legal, ext_versand_page</keywords>
<responsibilities>
  ...
  <item>Call extractVersandPage() and upsert one row per content_sha256 into ext_versand_page.</item>
  <item>Write extract-report.json artifact with counts.</item>
</responsibilities>
<non-goals>
  <item>Do not fetch pages — that is CrawlGogol's responsibility.</item>
  <item>Do not write to any table other than ext_versand_page.</item>
</non-goals>
</MODULE_CONTRACT>
<MODULE_MAP>
  <entry key="ExtractVersandPageGogol.run">Main extraction loop over page_observations for the current batch.</entry>
</MODULE_MAP>
<CHANGE_SUMMARY>
...
import { extractVersandPage } from '@org/business-crawler/extract';
import { ExtractGogolBase, type ObsRow } from './ExtractGogolBase.js';

export class ExtractVersandPageGogol extends ExtractGogolBase {
