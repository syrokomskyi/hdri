/*
<MODULE_CONTRACT>
<purpose>Detects WhatsApp contact links on crawled pages and writes results to ext_social_whatsapp.</purpose>
<keywords>extract, social, WhatsApp, wa.me, ext_social_whatsapp</keywords>
<responsibilities>
  ...
  <item>Call extractSocialWhatsapp() and upsert one row per content_sha256 into ext_social_whatsapp.</item>
  <item>Write extract-report.json artifact with counts.</item>
</responsibilities>
<non-goals>
  <item>Do not fetch pages — that is CrawlGogol's responsibility.</item>
  <item>Do not write to any table other than ext_social_whatsapp.</item>
</non-goals>
</MODULE_CONTRACT>
<MODULE_MAP>
  <entry key="ExtractSocialWhatsappGogol.run">Main extraction loop over page_observations for the current batch.</entry>
</MODULE_MAP>
<CHANGE_SUMMARY>
...
import { extractSocialWhatsapp } from '@org/business-crawler/extract';
import { ExtractGogolBase, type ObsRow } from './ExtractGogolBase.js';

export class ExtractSocialWhatsappGogol extends ExtractGogolBase {
  override readonly id = 'extract-social-whatsapp';
  override readonly table = 'ext_social_whatsapp';

  protected override extractDom($: CheerioAPI, _row: ObsRow): unknown[] | null {
    const r = extractSocialWhatsapp($);
