/*
<MODULE_CONTRACT>
<purpose>Detects links to Unternehmensregister on crawled pages and writes results to ext_link_unternehmensregister.</purpose>
<keywords>extract, Unternehmensregister, external link, ext_link_unternehmensregister</keywords>
<responsibilities>
  ...
  <item>Call extractLinkUnternehmensregister() and upsert one row per content_sha256 into ext_link_unternehmensregister.</item>
...
<non-goals>
  ...
  <item>Do not write to any table other than ext_link_unternehmensregister.</item>
</non-goals>
</MODULE_CONTRACT>
<MODULE_MAP>
  <entry key="ExtractLinkUnternehmensregisterGogol.run">Main extraction loop over page_observations for the current batch.</entry>
</MODULE_MAP>
<CHANGE_SUMMARY>
...
import { extractLinkUnternehmensregister } from '@org/business-crawler/extract';
import { ExtractGogolBase, type ObsRow } from './ExtractGogolBase.js';

export class ExtractLinkUnternehmensregisterGogol extends ExtractGogolBase {
  override readonly id = 'extract-link-unternehmensregister';
  override readonly table = 'ext_link_unternehmensregister';

  protected override extractDom($: CheerioAPI, _row: ObsRow): unknown[] | null {
    const r = extractLinkUnternehmensregister($);
  ...
  protected override get upsertSql(): string {
    return `INSERT INTO ext_link_unternehmensregister (content_sha256, extractor_ver, present, url) VALUES (?, ?, ?, ?)
