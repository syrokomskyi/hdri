/*
<MODULE_CONTRACT>
<purpose>Detects Facebook/fb.com profile links on crawled pages and writes results to ext_social_facebook.</purpose>
<keywords>extract, social, Facebook, fb.com, ext_social_facebook</keywords>
<responsibilities>
  ...
  <item>Call extractSocialFacebook() and upsert one row per content_sha256 into ext_social_facebook.</item>
  <item>Write extract-report.json artifact with counts.</item>
</responsibilities>
<non-goals>
  <item>Do not fetch pages — that is CrawlGogol's responsibility.</item>
  <item>Do not write to any table other than ext_social_facebook.</item>
</non-goals>
</MODULE_CONTRACT>
<MODULE_MAP>
  <entry key="ExtractSocialFacebookGogol.run">Main extraction loop over page_observations for the current batch.</entry>
</MODULE_MAP>
<CHANGE_SUMMARY>
...
import { extractSocialFacebook } from '@org/business-crawler/extract';
import { ExtractGogolBase, type ObsRow } from './ExtractGogolBase.js';

export class ExtractSocialFacebookGogol extends ExtractGogolBase {
  override readonly id = 'extract-social-facebook';
  override readonly table = 'ext_social_facebook';

  protected override extractDom($: CheerioAPI, _row: ObsRow): unknown[] | null {
    const r = extractSocialFacebook($);
  ...
  protected override get upsertSql(): string {
    return `INSERT INTO ext_social_facebook (content_sha256, extractor_ver, present, url) VALUES (?, ?, ?, ?)
