/*
<MODULE_CONTRACT>
<purpose>Detects membership in professional organizations (Verband, Innung, Kammer, etc.) on crawled pages and writes results to ext_memberships.</purpose>
<keywords>extract, memberships, Mitgliedschaft, Verband, Innung, ext_memberships</keywords>
<responsibilities>
  ...
  <item>Call extractMemberships() and upsert one row per content_sha256 into ext_memberships.</item>
  <item>Write extract-report.json artifact with counts.</item>
</responsibilities>
<non-goals>
  <item>Do not fetch pages — that is CrawlGogol's responsibility.</item>
  <item>Do not write to any table other than ext_memberships.</item>
</non-goals>
</MODULE_CONTRACT>
<MODULE_MAP>
  <entry key="ExtractMembershipsGogol.run">Main extraction loop over page_observations for the current batch.</entry>
</MODULE_MAP>
<CHANGE_SUMMARY>
...
import { extractMemberships } from '@org/business-crawler/extract';
import { ExtractGogolBase, type ObsRow } from './ExtractGogolBase.js';

export class ExtractMembershipsGogol extends ExtractGogolBase {
  override readonly id = 'extract-memberships';
  override readonly table = 'ext_memberships';

  protected override extractDom($: CheerioAPI, _row: ObsRow): unknown[] | null {
    const r = extractMemberships($);
  ...
  protected override get upsertSql(): string {
    return `INSERT INTO ext_memberships (content_sha256, extractor_ver, present, confidence) VALUES (?, ?, ?, ?)
