...
<keywords>sign, signature, ed25519, content-hash, traceability, core.db</keywords>
<responsibilities>
  <item>Computes SHA-256 hash of the final core.db file.</item>
...
<MODULE_MAP>
  <entry key="SignSourceGogol">Signs core.db file hash for downstream verification.</entry>
</MODULE_MAP>
<CHANGE_SUMMARY>
...
export class SignSourceGogol extends Gogol {
  ...
  override readonly guide = {
    title: 'Sign source',
    purpose: 'Create cryptographic signature over core.db file hash for downstream verification and traceability.',
  ...
  override async run(ctx: PipelineContext): Promise<void> {
    ...
    const contentHash = await hashDatabaseFile(coreDbPath);
    ...
    console.log(`[sign-source] core.db signed. hash=${contentHash} key=${manifest.signing_key_id}`);
  }
}
