...
export class DiscoverCoresGogol extends Gogol {
  ...
  override readonly guide = {
    ...
    purpose:
      "Scan sibling 0-harvest-source output directories to locate core_YYYY.db files across all devices.",
  ...
  override async run(ctx: PipelineContext): Promise<void> {
    ...
    await writeJsonFile(path.join(outputDir, "discovered-cores.json"), {
      ...
      [
        ...
        markdownTable(
          [
            ...
            ...discovered.map((d) => [
              d.deviceId,
              toFactoryRelativePath(d.dbPath),
              String(d.sizeBytes),
            ]),
          ],
          { align: ["l", "l", "r"] },
