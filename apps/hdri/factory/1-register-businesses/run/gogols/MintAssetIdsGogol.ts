/*
<MODULE_CONTRACT>
<purpose>Generates summary of minted deterministic asset IDs — this module handles mint asset ids operations within the pipeline application.</purpose>
<non-goals>
...
export class MintAssetIdsGogol extends Gogol {
  ...
  override readonly guide = {
    ...
    purpose:
      "Generate summary artifacts for all deterministic da-* asset IDs assigned to registered domains.",
    decisionType: "auto" as const,
    inputs: ["registry_YYYY.db business_registry table"],
    outputs: ["asset-id-summary.json", "asset-ids.ndjson", "asset-id-summary.md"],
    definitionOfDone: [
  ...
  override async run(ctx: PipelineContext): Promise<void> {
    ...
    await writeJsonFile(path.join(outputDir, "asset-id-summary.json"), {
      appId: "1-register-businesses",
      appVersion: APP_VERSION,
      deviceId: state.deviceId,
      sourceToken: state.sourceToken,
      assetIdsMinted: assetIdRows.length,
      deterministicRule: "deriveAssetId(domain) from @syrokomskyi/observatory-core",
    ...
    await writeTextFile(
      ...
      [
        renderKeyValueMd("Mint asset IDs", [
          ["Deterministic rule", "`deriveAssetId(domain)` from `@syrokomskyi/observatory-core`"],
        ...
        markdownTable(
          [
            ...
            ...assetIdRows
              .slice(0, 20)
              .map((row) => [row.daId, row.domain, String(row.sitesCount)]),
          ],
          { align: ["l", "l", "r"] },
        ),
      ].join("\n"),
