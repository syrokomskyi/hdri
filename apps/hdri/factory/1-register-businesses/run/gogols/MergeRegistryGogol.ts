...
export class MergeRegistryGogol extends Gogol {
  ...
  override readonly guide = {
    ...
    purpose:
      "Read distinct domains from all upstream core DBs, deduplicate across devices, and populate local business_registry with deterministic asset IDs.",
  ...
  override async run(ctx: PipelineContext): Promise<void> {
    ...
    const aggregates = Array.from(aggregateByDomain.values()).sort((a, b) =>
      a.daId.localeCompare(b.daId),
    );
    ...
    await writeJsonFile(path.join(outputDir, "merge-registry.json"), {
      ...
      [
        ...
        markdownTable(
          [
            ...
            ...perDeviceRows.map((row) => [
              row.deviceId,
              String(row.distinctDomains),
              toFactoryRelativePath(row.dbPath),
            ]),
          ],
          { align: ["l", "r", "l"] },
        ),
