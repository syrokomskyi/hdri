...
1. Walks every sibling `0-harvest-source/.output/<deviceId>/data/db/core_<sourceToken>.db` except devices whose folder name starts with `-` (ignored).
...
5. Writes the merged registry to: `.output/<DEVICE_ID>/data/db/registry_<sourceToken>.db`
...
6. Final gogol: `<N>-sign-source` produces a per-source ed25519 signature manifest covering the registry tables (Phase B).
...
`sourceToken` is read from the **shared factory-level** `factory/.input/brief.md`. No upstream paths needed — register-businesses scans all sibling `0-harvest-source/.output/*/` folders except those starting with `-`.
