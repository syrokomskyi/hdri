...
export class LivenessByBundeslandGogol extends Gogol {
  ...
  override readonly guide = {
    ...
    purpose:
      "Enrich liveness_checks with bundesland from registry.db and produce a per-state breakdown.",
  ...
  override async run(ctx: PipelineContext): Promise<void> {
    ...
    try {
      ...
      const columns = livenessDb.prepare("PRAGMA table_info(liveness_checks)").all() as Array<{
        name: string;
      }>;
