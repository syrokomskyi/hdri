...
export class SummarizeProfileGogol extends Gogol {
  ...
  private async loadGeoIndex(ctx: PipelineContext): Promise<GeoIndex> {
    ...
    if (!zipcodesPath) {
      throw new Error(
        "[summarize-profile] zipcodesTablePath is missing in brief.md — required for geographic reports",
      );
    }
    ...
    try {
      ...
      console.log(`[summarize-profile] Indexed ${postalToState.size} postal codes to states`);

      return { postalToState };
    } catch (error) {
      const err = error instanceof Error ? error : new Error(String(error));
      throw new Error(
        `[summarize-profile] Failed to load zipcodes data from ${fullPath}: ${err.message}`,
        { cause: error },
      );
    }
  }
  ...
  override async run(ctx: PipelineContext): Promise<void> {
    ...
    const uniqueContent = (
      db.prepare(`SELECT COUNT(DISTINCT content_sha256) AS n FROM page_observations`).get() as {
        n: number;
      }
    ).n;

    const extractions = (
      db.prepare(`SELECT COUNT(*) AS n FROM ext_impressum`).get() as { n: number }
    ).n;
    ...
    const openingHoursCount = (
      db.prepare(`SELECT COUNT(*) AS n FROM ext_opening_hours WHERE text IS NOT NULL`).get() as {
        n: number;
      }
    ).n;

    const copyrightYearCount = (
      db.prepare(`SELECT COUNT(*) AS n FROM ext_copyright_year WHERE year IS NOT NULL`).get() as {
        n: number;
      }
    ).n;

    const copyrightYearDist = db
      .prepare(
    ...
    const snapshot: Record<string, unknown> = {
      ...
      copyrightYearDistribution: Object.fromEntries(
        copyrightYearDist.map((r) => [String(r.year), r.n]),
      ),
      // New signal groups
    ...
    const renderDistTable = (header: [string, string], dist: Record<string, number>): string => {
      ...
      return markdownTable([header, ...sorted.map(([k, n]) => [k, String(n)])], {
        align: ["l", "r"],
      });
    };
    ...
    const render2DDistTable = (
      ...
      return markdownTable([tableHeader, ...rows], {
        align: ["l", "r", ...allGroups.map(() => "r" as const)],
      });
    };
