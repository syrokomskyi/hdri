...
### Changed
- Regenerate all AGENTS.md files according to RFC-0070 step 16.

### Fixed
- Exclude test files from HDRI factory app tsconfigs.
- Remove spec/ files containing leaked AWS STS credentials.
- Fix routing logic for nested external domains as per RFC-0069.
- Align ontology version in HDRI observatory brief and update stale sourceToken in docs.
- Update brief, seal Q2 capsule, and generate prior-capsules.json for Q3 readiness in HDRI.

### Removed
- Remove spec/ files containing leaked AWS STS credentials.

### Security
- Remove files containing leaked AWS STS credentials to mitigate security risk.

### Documentation
- Update RUNBOOK troubleshooting and add guard recovery instructions.
- Document minSitesThreshold in brief.example.md.
- Add cross-app brief reading rule to factory AGENTS.md.

## 2026-07-23 .. 2026-07-29

### Added
- Restore missing package.json files across all packages and apps.
- Add new README and update AGENTS.md for consolidated agent documentation.

### Changed
- Upgrade and align dependencies across the entire workspace.
- Align vitest versions for consistent testing environment.

### Fixed
- Eliminate all pnpm install warnings in the monorepo.

### Removed
- Remove code-compass from all devDependencies and scripts.
- Flatten project structure by moving apps/source/* and related files to the root directory.

### Documentation
- Merge agent documentation and add project narrative for improved onboarding.
