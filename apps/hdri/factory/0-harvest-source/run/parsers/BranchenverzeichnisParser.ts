/*
<MODULE_CONTRACT>
<purpose>Independent HTML parser for the branchenverzeichnis.org source — this module handles branchenverzeichnis parser operations within the pipeline application.</purpose>
<non-goals>
...
export class BranchenverzeichnisParser implements SourceParser {
  ...
  private parseVcardPage(
    ...
    const businessName =
      $vcard.find(".org").first().text().trim() || $vcard.find(".fn").first().text().trim() || null;
  ...
  private parseCatalogPage(
    ...
    $("div.CompanyEntryRow").each((_, el) => {
      ...
      const city = this.extractCityFromUrl(websiteUrl) ?? this.extractCityFromName(businessName);
