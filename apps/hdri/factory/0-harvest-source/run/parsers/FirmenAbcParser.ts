/*
<MODULE_CONTRACT>
<purpose>Independent HTML parser for the firmenabc.com source — this module handles firmen abc parser operations within the pipeline application.</purpose>
<non-goals>
  ...
  parse(content: string, fileName: string): SourceParseResult {
    ...
    $(".listing, .entry").each((_, el) => {
      ...
      const websiteUrl =
        $el
          .find('a[href^="http"]')
          .filter((_, a) => !$(a).attr("href")?.includes("firmenabc.com"))
          .first()
          .attr("href") || null;
