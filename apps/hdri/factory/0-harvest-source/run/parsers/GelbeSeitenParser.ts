/*
<MODULE_CONTRACT>
<purpose>Independent HTML parser for the gelbeseiten.de source — this module handles gelbe seiten parser operations within the pipeline application.</purpose>
<non-goals>
  ...
  parse(content: string, fileName: string): SourceParseResult {
    ...
    $("article.result, .serp-listing").each((_, el) => {
      ...
      const websiteUrl = this.normalizeUrl(
        $el.find("a.homepage, a[data-follow-link]").first().attr("href") || null,
      );
