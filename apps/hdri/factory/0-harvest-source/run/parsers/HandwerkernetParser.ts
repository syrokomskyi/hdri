/*
<MODULE_CONTRACT>
<purpose>Independent HTML parser for the handwerkernet.de source — this module handles handwerkernet parser operations within the pipeline application.</purpose>
<non-goals>
...
export class HandwerkernetParser implements SourceParser {
  ...
  parse(content: string, fileName: string): SourceParseResult {
    ...
    $('[itemtype="https://schema.org/LocalBusiness"]').each((_, el) => {
      ...
      const businessName = $el.find('[itemprop="name"]').first().text().trim() || null;
      ...
      const rawUrl =
        $el.find('[itemprop="url"]').attr("href") || $el.find(".url a").attr("href") || null;
      ...
      const streetAddress = $el.find('[itemprop="streetAddress"]').first().text().trim() || null;
      const postalCode = $el.find('[itemprop="postalCode"]').first().text().trim() || null;
      const city = $el.find('[itemprop="addressLocality"]').first().text().trim() || null;
