...
import {
  ...
  getHwoMappingGroups,
  type HwoUid,
} from "@syrokomskyi/business-core/gewerk";
...
export class ClassifyBrancheGogol extends Gogol {
  ...
  override async run(ctx: PipelineContext): Promise<void> {
    ...
    console.log(`[classify-branche] Classifying ${sites.length} sites`);
    ...
    db.transaction(() => {
      for (const site of sites) {
        ...
        updateSiteStmt.run(result.uid, result.confidence, result.provenance, site.site_id);
    ...
    const destatisRows = destatisGroups.map((g) => [
      g.label,
      g.code,
      String(destatisDistribution[g.code] ?? 0),
    ]);
    ...
    const md: string[] = [
      ...
      markdownTable(
        [
          ...
          ["With HWO UID", String(classifiedWithUid)],
          ["Group only", String(classifiedWithGroupOnly)],
      ...
      ``,
      markdownTable([["Group", "Code", "Count"], ...destatisRows], { align: ["l", "l", "r"] }),
    ];
