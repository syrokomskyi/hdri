...
export class SnapshotHarvestGogol extends Gogol {
  ...
  private async loadGeoIndex(ctx: PipelineContext): Promise<GeoIndex> {
    ...
    if (!zipcodesPath) {
      throw new Error(
        "[snapshot-harvest] zipcodesTablePath is missing in root brief.md — required for geographic reports",
      );
    }
    ...
    try {
      ...
      console.log(`[snapshot-harvest] Indexed ${postalToState.size} postal codes to states`);

      return { postalToState };
    } catch (error) {
      const err = error instanceof Error ? error : new Error(String(error));
      throw new Error(
        `[snapshot-harvest] Failed to load zipcodes data from ${fullPath}: ${err.message}`,
        { cause: error },
      );
    }
  }
  ...
  override async run(ctx: PipelineContext): Promise<void> {
    ...
    const totalSeeds = (
      db.prepare("SELECT COUNT(*) AS n FROM site_source_seeds").get() as { n: number }
    ).n;

    // All sites now have a Destatis group (either canonical I–VII or 'unclassified')
    const gewerkDistRows = db
      .prepare(
    ...
    const getAllGroups = (dist: Record<string, Record<string, number>>): string[] => {
      ...
      const normal = Array.from(groups)
    ...
    const mdSections: string[] = [
      ...
      markdownTable(
        [
          ["Metric", "Value"],
          ["Total unique sites", String(totalSites)],
          ["Seeds this batch", String(totalSeeds)],
        ],
        { align: ["l", "r"] },
      ),
      ``,
      ...
      markdownTable(
        [
          ["Group", "Sites"],
          ...sortedCanonical.map(([g, n]) => [g, String(n)]),
          ["unclassified", String(unclassifiedCount)],
        ],
        { align: ["l", "r"] },
      ),
    ];
