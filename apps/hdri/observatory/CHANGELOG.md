...
### Added
- Restore missing package.json files across multiple packages and add a top-level README file.

### Changed
- Upgrade dependencies across the workspace for improved compatibility and performance.
- Align all vitest versions to ensure consistent testing infrastructure.

### Fixed
- Flatten project structure by moving apps/source/* to the root directory and cleaning up residual files.
- Eliminate all pnpm install warnings to ensure a clean installation process.

### Removed
- Remove code-compass from all package.json devDependencies and scripts.
- Remove deprecated per-kind write wrappers in observatory-vault.

### Documentation
- Merge AGENTS.md and update docs/compass-inventory.xml to reflect structural changes.
