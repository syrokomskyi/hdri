...
export class SetupObservatoryRunGogol extends Gogol {
  ...
  override async run(ctx: PipelineContext): Promise<void> {
    ...
    try {
      const migration = migrateObservatory(db);
      if (migration.backupPath) {
        log.info(
          "pre-migration-backup",
          `Protective DB backup written before a destructive migration: ${migration.backupPath}`,
      ...
      if (migration.applied.length > 0) {
        log.info(
          "migrations-applied",
          `Applied ${migration.applied.length} schema migration(s): ` +
            migration.applied.map((m) => `${m.id}:${m.name}`).join(", "),
...
async function checkPreviousPeriod(
  ...
  try {
    ...
    log.warn(
      "no-previous-db",
      `No DB found for previous period ${prev} — first run for this year?`,
      { previousPeriod: prev },
    );
    return;
  }
  ...
  try {
    const row = db
      .prepare(`SELECT status FROM pipeline_runs WHERE period = ? ORDER BY started_at DESC LIMIT 1`)
      .get(prev) as { status: string } | undefined;

    if (!row) {
      log.warn("no-previous-run", `No pipeline run found for previous period ${prev}.`, {
        previousPeriod: prev,
      });
    } else if (row.status !== "finished") {
      log.warn(
        "previous-not-finished",
        `Previous period ${prev} has status "${row.status}" (not "finished"). Consider completing it before running ${period}.`,
        {
          previousPeriod: prev,
...
function previousPeriod(period: string): string | null {
  const m = period.toLowerCase().match(/^(\d{4})-q([1-4])$/);
  if (!m) return null;
