...
export class BuildCohortsGogol extends Gogol {
  ...
  override async run(ctx: PipelineContext): Promise<void> {
    ...
    try {
      ...
      // Aggregate
      const report = aggregateCohort(scoredSites, dimensionIds, [
        "strataCode",
        "gewerkGroup",
        "bundesland",
      ]);
      ...
      const writeCohort = db.transaction(() => {
        // Idempotent rebuild: drop any prior cohorts (and their members and
        // aggregates) for this run before inserting, so re-running build-cohorts
        // for the same run_id replaces rather than accumulates.
        db.prepare(
          `DELETE FROM cohort_aggregates WHERE cohort_id IN (SELECT id FROM cohorts WHERE run_id = ?)`,
        ).run(runId);
        db.prepare(
          `DELETE FROM cohort_members WHERE cohort_id IN (SELECT id FROM cohorts WHERE run_id = ?)`,
        ).run(runId);
        db.prepare(`DELETE FROM cohorts WHERE run_id = ?`).run(runId);
      ...
      log.info(
        "cohort-finished",
        `Done. Cohort ${cohortId}: ${scoredSites.length} members, ${dimensionIds.length} dimensions.`,
        {
          cohortId,
