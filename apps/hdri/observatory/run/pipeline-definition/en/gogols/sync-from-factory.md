...
Consumes factory emit-bundles (manifest.json + observations.ndjson) produced by `EmitObservationsGogol` in `3-extract-profile` and writes them to the observatory observations table.

The `synced_bundles` table tracks ingested `run_id` values so re-running the observatory is safe — already-synced bundles are skipped.
