...
Reads signed observations (signature IS NOT NULL) for each factory run synced in this observatory run, then writes one ZSTD Parquet shard per factory run to the vault at `brief.vaultDir` (defaults to `.output/vault/`).
