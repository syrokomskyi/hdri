...
const createEmitBundle = (
  ...
  overrides: Partial<import("@syrokomskyi/observatory-emit").EmitManifest> = {},
): string => {
...
describe("SyncFromFactory — DB insertion", () => {
  ...
  it("inserts observations into the observatory schema", async () => {
    ...
    const count = (obsDb.prepare("SELECT COUNT(*) as c FROM observations").get() as { c: number })
      .c;
    expect(count).toBe(2);
    ...
    const row = obsDb
      .prepare(
        "SELECT signal_path, value_bool, value_num, value_str, value_json, obs_json, collection_status FROM observations LIMIT 1",
      )
  ...
  it("is idempotent — INSERT OR IGNORE on duplicate observation_id", async () => {
    ...
    const count1 = (obsDb.prepare("SELECT COUNT(*) as c FROM observations").get() as { c: number })
      .c;
    expect(count1).toBe(4); // 2 from previous + 2 new (dup IDs but different run_id)

    // Insert again with same IDs → no changes
    insertObservations(obsDb, bundle.manifest.run_id, all, obsDbRunId);
    const count2 = (obsDb.prepare("SELECT COUNT(*) as c FROM observations").get() as { c: number })
      .c;
  ...
  it("tracks synced bundles idempotently", async () => {
    ...
    const synced = obsDb
      .prepare("SELECT run_id FROM synced_bundles WHERE run_id = ?")
  ...
  it("handles observations with all value types", async () => {
    const obsBool = makeObservation({
      signal_path: "test.bool",
      value_bool: true,
      value_type: "bool",
    });
    const obsNum = makeObservation({ signal_path: "test.num", value_num: 42, value_type: "num" });
    const obsStr = makeObservation({
      signal_path: "test.str",
      value_str: "hello",
      value_type: "str",
    });
    const obsJson = makeObservation({
      signal_path: "test.json",
      value_json: '{"a":1}',
      value_type: "json",
    });
    ...
    const boolRow = obsDb
      .prepare("SELECT value_bool FROM observations WHERE signal_path = 'test.bool'")
      .get() as { value_bool: number | null };
    expect(boolRow.value_bool).toBe(1);

    const numRow = obsDb
      .prepare("SELECT value_num FROM observations WHERE signal_path = 'test.num'")
      .get() as { value_num: number | null };
    expect(numRow.value_num).toBe(42);

    const strRow = obsDb
      .prepare("SELECT value_str FROM observations WHERE signal_path = 'test.str'")
      .get() as { value_str: string | null };
    expect(strRow.value_str).toBe("hello");

    const jsonRow = obsDb
      .prepare("SELECT value_json FROM observations WHERE signal_path = 'test.json'")
