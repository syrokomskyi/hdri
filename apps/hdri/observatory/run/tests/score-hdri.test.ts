...
function crossValidateCodebook(codebook: Codebook, ontology: SignalOntology): void {
  ...
  if (unknown.length > 0) {
    throw new Error(`Codebook references signals not in ontology:\n  ` + unknown.join("\n  "));
  }

  if (deprecated.length > 0) {
    const log = createJsonLogger({ app: "observatory", gogol: "score-hdri-test" });
    log.warn("deprecated-signals", `${deprecated.length} deprecated signal(s) in codebook`, {
...
function buildIndicatorIndex(
  codebook: Codebook,
): Map<string, { remediation?: unknown; source?: { extractor?: string } }> {
  ...
  it("groups observations by asset_id", () => {
    ...
    const rows: ObsRow[] = [
      ...
      },
      {
        id: "obs-2",
        asset_id: assetA,
        signal_path: "legal.datenschutz.present",
        value_bool: 0,
        value_num: null,
        value_str: null,
        value_json: null,
...
describe("ScoreHdri — codebook cross-validation", () => {
  ...
  it("validates codebook inputKeys against ontology", async () => {
    ...
    const codebook = parseCodebookOrThrow(fs.readFileSync(CODEBOOK_PATH, "utf-8"), CODEBOOK_PATH);
...
describe("ScoreHdri — scoring with real codebook", () => {
  it("scores an asset with complete signals", () => {
    ...
    // One scored entry per codebook dimension (currently 6: legal, contact,
    // structured, trust, social, accessibility). Tied to the codebook so adding
    // a dimension doesn't silently rot this assertion.
    expect(result.dimensions.length).toBe(codebook.dimensions.length);
...
describe("ScoreHdri — indicator trace enrichment", () => {
  it("populates declared_extractor from codebook indicator source", () => {
    ...
    // The operational codebook (v1.1+) declares source.extractor per indicator;
    // impressum is extracted by rule_v3. This is what populates declared_extractor.
    expect(impressum?.source?.extractor).toBe("rule_v3");
  });
