/*
<MODULE_CONTRACT>
<purpose>Assembles the observatory pipeline from markdown declarations — this module handles pipeline operations within the pipeline application.</purpose>
<non-goals>
...
} from "./pipeline/declaration";
import type { PipelineBuildContext, ObservatoryPipelineStep } from "./pipeline/build-types";
import { createPhaseById, isPhaseId } from "./pipeline/phase-registry";

type AppPipelineDefinition = PipelineDefinition<ObservatoryPipelineStep>;

const createExecutionSummary = (): string =>
  "Digital Observatory pipeline: harvest → observe → interpret → publish. " +
...
export const createGogols = (): ObservatoryPipelineStep[] => createPipeline().steps;
