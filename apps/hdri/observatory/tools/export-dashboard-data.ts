/*
<MODULE_CONTRACT>
<purpose>Exports privacy-safe HDRI dashboard JSON and debug CSV files from observatory output into the dashboard Astro app assets.</purpose>
<non-goals>
...
<CHANGE_SUMMARY>
  ...
  <item>Pass policy from main() to writeEmptyPayloads; use safe defaults on degraded no-DB path instead of calling loadKAnonPolicy() again.</item>
</CHANGE_SUMMARY>
*/
...
const DASHBOARD_APP_DIR = path.resolve(process.cwd(), "../dashboard");
...
async function main(): Promise<void> {
  ...
  if (!dbPath) {
    await writeEmptyPayloads(undefined, policy);
  ...
  try {
    ...
    if (!run) {
      await writeEmptyPayloads(dbPath, policy);
    ...
    const dimensionsExport = buildDimensions(dimensions, kAnonymityMin);
    ...
    const matrix = buildMatrix(scores, kAnonymityMin);
    const manifest: Manifest = {
      generatedAt: new Date().toISOString(),
      effectiveKMin: policy.effective_k_min,
      hardFloor: policy.hard_floor,
      period: run.period,
      runId: run.run_id,
      ...
      totalPublishedSlices:
        dimensionsExport.length + bundeslaender.length + gewerke.length + matrix.length,
    };
...
function buildDimensions(rows: DimensionRow[], kAnonymityMin: number): DimensionExport[] {
...
function buildMatrix(rows: ScoreRow[], kAnonymityMin: number): MatrixCell[] {
  const grouped = new Map<string, number[]>();
  for (const row of rows) {
...
function buildMaturity(
  scores: number[],
): Array<{ id: string; label: string; count: number; share: number }> {
...
async function writeJson(fileName: string, data: unknown): Promise<void> {
  await fs.writeFile(
    path.join(DASHBOARD_DATA_DIR, fileName),
    JSON.stringify(data, null, 2),
    "utf-8",
  );
}

async function writeEmptyPayloads(dbPath?: string, policy?: KAnonPolicy): Promise<void> {
  const manifest: Manifest = {
    generatedAt: new Date().toISOString(),
    effectiveKMin: policy?.effective_k_min ?? 5,
    hardFloor: policy?.hard_floor ?? 5,
    period: null,
    runId: null,
...
void main().catch((error: unknown) => {
  log.error("failed", "[export-dashboard-data] Failed:", { error: String(error) });
  process.exitCode = 1;
});
