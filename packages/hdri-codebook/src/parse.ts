/*
<MODULE_CONTRACT>
<purpose>Validates and parses codebooks using Zod schemas and semantic rules.
Exports Zod-derived input types as the single source of truth for codebook structures.</purpose>
<non-goals>
  <item>Does not handle file I/O operations for reading codebooks.</item>
  <item>Does not modify or transform codebook data beyond validation.</item>
  <item>Does not define output types (SiteScore, DimensionScore, IndicatorTrace) — those live in types.ts.</item>
</non-goals>
</MODULE_CONTRACT>
<CHANGE_SUMMARY>
  <item>Initial implementation of codebook parsing and validation.</item>
  <item>Export Zod-derived input types (ScoringRule, Codebook, Indicator, etc.) — single source of truth.</item>
  <item>Remove import of Codebook from types.ts — now defined locally via z.infer.</item>
</CHANGE_SUMMARY>
*/
...
// ---------------------------------------------------------------------------
// Zod-derived input types (single source of truth — Candidate 1)
// ---------------------------------------------------------------------------

export type ScoringRule = z.infer<typeof ruleSchema>;
export type BoolRule = Extract<ScoringRule, { type: "bool" }>;
export type PresenceRule = Extract<ScoringRule, { type: "presence" }>;
export type CountClampRule = Extract<ScoringRule, { type: "countClamp" }>;
export type CountClampInverseRule = Extract<ScoringRule, { type: "countClampInverse" }>;
export type EnumRule = Extract<ScoringRule, { type: "enum" }>;

export type MissingPolicy = z.infer<typeof missingSchema>;
export type ConditionalMissingState = z.infer<typeof conditionalMissingStateSchema>;
export type ConditionalMissingPolicy = Extract<MissingPolicy, { kind: "conditional" }>;

export type SignalSource = z.infer<typeof sourceSchema>;
export type RemediationMetadata = z.infer<typeof remediationSchema>;
...
const checkSemantics = (cb: Codebook): void => {
  ...
  for (const d of cb.dimensions) {
    ...
    for (const i of d.indicators) {
      ...
      if (
        (i.rule.type === "countClamp" || i.rule.type === "countClampInverse") &&
        i.rule.max <= i.rule.min
      ) {
...
export const parseCodebook = (source: string): Codebook => {
  ...
  const parsed = codebookSchema.parse(raw);
