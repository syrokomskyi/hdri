/*
<MODULE_CONTRACT>
<purpose>Implements a three-state circuit breaker to manage fault tolerance in asynchronous operations by transitioning between closed, open, and half-open states based on failure rates.</purpose>
<non-goals>
  <item>Does not provide persistence for breaker state across application restarts.</item>
  <item>Does not handle synchronous operations directly.</item>
</non-goals>
</MODULE_CONTRACT>
<CHANGE_SUMMARY>
  <item>Initial implementation of the circuit breaker with configurable options and state transitions.</item>
  <item>Replaced bare now() injection with unified Clock interface from clock.ts.</item>
</CHANGE_SUMMARY>
*/
...
export class CircuitBreaker {
  ...
  private maybeTransitionToHalfOpen(): void {
    ...
    if (this.clock.now() - this.openedAtMs >= this.cooldownMs) {
