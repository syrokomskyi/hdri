...
### Added
- Add AGENTS.md documentation files with agent details to all major packages and apps.
- Add changelog links to all README.md files for improved navigation.
- Generate CHANGELOG.md files for all packages and apps.

### Changed
- Migrate forge dependency from @webgogol/forge@0.3.0 to @warpgogol/forge@0.7.0 across the project.
- Rename @wgogol/changelog-live package to @warpgogol/changelog-live in all relevant files.
- Upgrade dependencies: typescript to 7.0.2, eslint to 10.8.0, @playwright/test to 1.62.1, commander to 15.0.0, openai to 7.4.0, @anthropic-ai/sdk to 0.115.0, jsdom to 30.0.1, write-file-atomic to 8.0.0, @types/better-sqlite3 to 9.6.0, @tanstack/table-core to 9.0.0, @swc/core to 1.15.47, @swc/helpers to 0.5.23, @swc-node/register to 1.12.1, @warpgogol/forge to 0.9.0, cross-env to 10.1.0, jiti to 2.7.0, turbo to 2.10.8, and various project dependencies.
- Upgrade other dependencies: jose, tsx, typescript-eslint, csv-parse/stringify, undici, verdaccio, forge and related package versions.

### Fixed
- Downgrade TypeScript from 7.0.2 to 6.0.3 for compatibility with typescript-eslint.

### Documentation
- Update and add multiple documentation files, principles, and agent-related content throughout the codebase.

## 2026-07-23 .. 2026-07-29

### Added
- Implement 'light-v2' tier for the 'truth' pipeline with epistemic diversity, structural improvements, and supporting prompt and documentation files.
- Extract Perplexity citations from API responses and scan AI response files for URLs.
- Add README file and restore missing package.json files across multiple packages and apps.

### Changed
- Upgrade dependencies across all workspace packages and apps.
- Refactor pipeline-ai to extract runWithOptionalLogging utility, reducing logging boilerplate.
- Align Vitest versions across packages.
- Update 'truth' pipeline logic, types, and registry for new tier and process changes.

### Fixed
- Restore package.json files that were previously missing.
- Merge AGENTS.md into project root.

### Removed
- Remove code-compass from all package.json devDependencies and scripts.
- Flatten project structure by moving content from apps/source/* to the project root, removing redundant subfolders and configuration files.

### Documentation
- Document 'truth' light-v2 tier and escalation gate.
- Update cross-model-analysis and pipeline documentation for structural changes.
