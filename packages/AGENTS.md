...
- `packages/repo-extract`: Extracts apps and packages from a TurboRepo monorepo into standalone public repositories. Provides programmatic API (`extractProject()`) and CLI (`repo-extract`). Configured via per-project `extract.config.yaml` with Zod validation. Supports monorepo and standalone export modes, declarative post-process rules (copy/patch/delete), and optional `@warpgogol/changelog-live` integration. Published as `@warpgogol/repo-extract`.
