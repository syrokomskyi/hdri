...
- The `@warpgogol/` scope is reserved for packages published to NPM for external use (e.g. `@warpgogol/forge`, `@warpgogol/repo-extract`, `@warpgogol/changelog-live`). Use `@syrokomskyi/` for internal packages and `@warpgogol/` for publishable ones.
...
- `packages/axiom/axiom-capture`: public capture/schedule contracts, deterministic keys, bounded discovery, live HTTP/Playwright/Browsertrix/replay adapters, evidence indexes, closure/seal guards, and local-dev site evidence capture (site-evidence graphs, check targets, safety validation, Playwright capture adapter, diagnostic types, run paths, reports, action packs, audience profiles). `PlaywrightEvidenceDriver.capture()` collects 6 evidence roles beyond accessibility: performance-metrics, console-errors, page-errors, http-response-headers, cookie-state, network-requests. `playwright` is a peerDependency.
- `packages/axiom/axiom-methodology`: immutable methodology packages, validation, signed governance, compatibility, epistemic contracts, and the `runActiveMethodologies()` orchestrator that encapsulates the full pipeline (capture contract → discovery → capture → extraction → instruments → findings → study-run). Runtime dependencies: `axiom-capture`, `axiom-study`.
- `packages/axiom/axiom-study`: real instruments (pure functions), evidence extraction functions (`CapturedEvidence[]` → instrument state arrays), observation/study persistence interfaces and in-memory stores, findings, uncertainty, cache/fingerprint/timeline. PostgreSQL store implementations live in `axiom-runtime`. Strict leaf — no runtime dependencies. `@syrokomskyi/axiom-capture` is a devDependency (type-only import of `CapturedEvidence`).
...
- `packages/changelog-live`: AI-powered CHANGELOG.md generator from git history. Collects commits from configured paths (including historical rename paths), groups by configurable periods (day, week, biweekly, month), generates changelog entries via OpenAI/Anthropic/Gemini, and writes multi-language CHANGELOG files. Includes `changelog-live init` subcommand that traces git rename history to discover all historical paths and generates `changelog.config.yaml`. Published as `@warpgogol/changelog-live`.
- `packages/repo-extract`: Extracts apps and packages from a TurboRepo monorepo into standalone public repositories. Provides programmatic API (`extractProject()`) and CLI (`repo-extract`). Configured via per-project `extract.config.yaml` with Zod validation. Supports monorepo and standalone export modes, declarative post-process rules (copy/patch/delete), optional `@warpgogol/changelog-live` integration, and multi-package-manager support (pnpm, npm, yarn) with auto-detection via lockfile presence. Published as `@warpgogol/repo-extract`.
