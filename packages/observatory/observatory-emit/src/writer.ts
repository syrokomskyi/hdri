/*
<MODULE_CONTRACT>
<purpose>Streams observations and asset states to a specified directory in NDJSON format for later processing.</purpose>
<non-goals>
  <item>Does not handle the reading or processing of the NDJSON data once written.</item>
  <item>Does not provide mechanisms for data validation beyond serialization.</item>
  <item>Does not manage concurrent access or modifications to the output directory.</item>
</non-goals>
</MODULE_CONTRACT>
<CHANGE_SUMMARY>
  <item>Initial implementation of EmitBundleWriter for streaming data to NDJSON files.</item>
</CHANGE_SUMMARY>
*/
...
export class EmitBundleWriter {
  ...
  async commit(): Promise<EmitManifest> {
    ...
    const assetStatesHash = this.assetStates.count > 0 ? this.assetStates.hash.digest("hex") : null;
    const evidenceCount = await countEvidenceFiles(path.join(this.emitDir, "evidence"));
  ...
  private finaliseTracker(t: FileTracker): Promise<void> {
    return new Promise<void>((resolve, reject) => {
      t.stream.end((err: Error | null | undefined) => (err ? reject(err) : resolve()));
    });
  }
}
