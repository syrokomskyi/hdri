/*
<MODULE_CONTRACT>
<purpose>Facilitates multi-device signature verification by loading public keys and verifying manifests.</purpose>
<non-goals>
  <item>Does not handle key generation or storage beyond loading existing keys.</item>
</non-goals>
</MODULE_CONTRACT>
<CHANGE_SUMMARY>
...
/**
 * Deep entry point: loads verification keys from the transparency directory
 * and verifies a source-signature manifest in one call.
 *
 * @param manifest The parsed SourceSignatureManifest to verify.
 * @param opts Optional configuration — keysDir defaults to getTransparencyKeysDir().
 * @throws if the keys directory cannot be read.
 */
export async function verifyUpstream(
  manifest: SourceSignatureManifest,
  opts: { keysDir?: string } = {},
): Promise<UpstreamVerificationResult> {
  const keysDir = opts.keysDir ?? getTransparencyKeysDir();
  const keyMap = await loadVerificationKeys(keysDir);
  return verifyUpstreamManifest(manifest, keyMap);
}
