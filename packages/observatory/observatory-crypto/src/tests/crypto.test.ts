...
import { verifyObservation, verifyObservations, verifySignedObservation } from "../verify.js";
...
// ---------------------------------------------------------------------------
// Trust-aware verification (verifySignedObservation)
// ---------------------------------------------------------------------------

const trustedEntry = (over: Partial<TrustedKeyEntry> = {}): TrustedKeyEntry => ({
  signingKeyId: "test-key-2026",
  deviceId: "test-pc",
  pemFile: "test-pc.pem",
  sha256: "abc",
...
const trustedManifest = (keys: TrustedKeyEntry[]): TrustedKeysManifest => ({
...
describe("verifySignedObservation", () => {
  it("validates when no manifest is given (crypto-only)", () => {
    const key = makeKey();
    const signed = signObservation(FIXTURE_OBS, key);
    const vk = { publicKeyPem: key.publicKeyPem, signingKeyId: key.signingKeyId };

    const result = verifySignedObservation(signed, vk);
    expect(result.valid).toBe(true);
    expect(result.reason).toBe("valid");
  });

  it("validates when manifest trusts the key", () => {
    const key = makeKey();
    const signed = signObservation(FIXTURE_OBS, key);
    const vk = { publicKeyPem: key.publicKeyPem, signingKeyId: key.signingKeyId };
    const manifest = parseTrustedKeysManifest(
      trustedManifest([trustedEntry({ signingKeyId: key.signingKeyId })]),
    );

    const result = verifySignedObservation(signed, vk, manifest);
    expect(result.valid).toBe(true);
  });

  it("rejects when manifest does not contain the key", () => {
    const key = makeKey();
    const signed = signObservation(FIXTURE_OBS, key);
    const vk = { publicKeyPem: key.publicKeyPem, signingKeyId: key.signingKeyId };
    const manifest = parseTrustedKeysManifest(trustedManifest([]));
  ...
  it("rejects when key is revoked", () => {
    const key = makeKey();
    const signed = signObservation(FIXTURE_OBS, key);
    const vk = { publicKeyPem: key.publicKeyPem, signingKeyId: key.signingKeyId };
    const manifest = parseTrustedKeysManifest(
      trustedManifest([
        trustedEntry({ signingKeyId: key.signingKeyId, status: "revoked", note: "compromised" }),
      ]),
    );
  ...
  it("rejects a bad signature even when key is trusted", () => {
    const key = makeKey();
    const signed = signObservation(FIXTURE_OBS, key);
    const tampered = { ...signed, value_bool: false };
    const vk = { publicKeyPem: key.publicKeyPem, signingKeyId: key.signingKeyId };
    ...
    const result = verifySignedObservation(tampered, vk, manifest);
    expect(result.valid).toBe(false);
