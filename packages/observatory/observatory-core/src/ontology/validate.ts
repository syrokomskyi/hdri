/*
<MODULE_CONTRACT>
<purpose>Validates signal paths, value types, and observations against a loaded ontology for both pipeline and test use.</purpose>
<non-goals>
  <item>Does not modify the ontology or observation data.</item>
</non-goals>
</MODULE_CONTRACT>
<CHANGE_SUMMARY>
  <item>Initial implementation of ontology validation logic.</item>
  <item>Use countPopulatedValues from value.ts — the exactly-one invariant is now owned by one module.</item>
</CHANGE_SUMMARY>
*/
...
import { countPopulatedValues } from "../value.js";
...
export const isValidPathFormat = (path: string): boolean => SIGNAL_PATH_RE.test(path);
...
export const validateObservation = (
  ...
  const populated = countPopulatedValues(obs);
...
export const validateObservations = (
  ...
  const allIssues = observations.flatMap((obs) => validateObservation(obs, ontology));
  return {
 ...
 */
export const isActiveSignal = (path: string, ontology: SignalOntology): boolean => {
