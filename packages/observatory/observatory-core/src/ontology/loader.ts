/*
<MODULE_CONTRACT>
<purpose>Parses ontology documents from YAML or JSON into a typed SignalOntology using a shared schema.</purpose>
<non-goals>
  <item>Does not modify or create ontology documents.</item>
  <item>Does not handle ontology validation beyond schema parsing.</item>
</non-goals>
</MODULE_CONTRACT>
<CHANGE_SUMMARY>
  <item>Initial implementation of ontology parsing and file reading functions.</item>
</CHANGE_SUMMARY>
*/
 ...
 *   - YAML (authoring format, e.g. apps/hdri/observatory/.input/ontology.yaml)
 *
...
export const parseOntology = (source: string, pathHint?: string): SignalOntology => {
  ...
  try {
    ...
    throw new Error(`${prefix}${err instanceof Error ? err.message : String(err)}`, { cause: err });
  }
};
