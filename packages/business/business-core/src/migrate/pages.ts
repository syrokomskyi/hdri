/*
<MODULE_CONTRACT>
<purpose>This module provides idempotent database migration functions for managing schema and metadata of pages_{YYYY}.db, ensuring all necessary tables and indexes are created for storing crawled page content and associated metadata.</purpose>
<non-goals>
  <item>This module does not handle actual data crawling or content extraction processes.</item>
</non-goals>
</MODULE_CONTRACT>
<CHANGE_SUMMARY>
  <item>Added idempotent column addition logic for extended page_observations schema.</item>
</CHANGE_SUMMARY>
*/

import type Database from "better-sqlite3";
export { stampSchemaMeta as stampPagesMeta } from "../schema/schema-meta.js";
...
export const migratePages = (db: Database.Database): void => {
  db.exec(`
    ...
    -- Impressum contact details (names, address, phone, email, USt-IdNr).
    -- FACTORY-LOCAL PII: deliberately NOT in EXT_SIGNAL_MAP, so it is never
    -- bridged into observations / HDRI / the published dashboard. Keyed by the
    -- fetched Impressum page's content hash. Populated only when the operator
    -- opts in (brief.collectImpressumContacts).
    CREATE TABLE IF NOT EXISTS ext_impressum_contacts (
      content_sha256  TEXT PRIMARY KEY,
      extractor_ver   TEXT NOT NULL,
      company_name    TEXT,
      person_names    TEXT,
    ...
    CREATE INDEX IF NOT EXISTS ext_impressum_contacts_ver_idx ON ext_impressum_contacts(extractor_ver);
