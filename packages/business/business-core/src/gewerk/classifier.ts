/*
<MODULE_CONTRACT>
<purpose>This module provides classification of business entries into HWO UIDs and mapping targets using a two-layer approach.</purpose>
<non-goals>
  <item>This module does not handle the persistence of classification results.</item>
</non-goals>
</MODULE_CONTRACT>
<CHANGE_SUMMARY>
  <item>Initial implementation of the HWO entry classification module with primary and fallback classifiers.</item>
</CHANGE_SUMMARY>
*/
...
import { getHwoEntry, listHwoEntries, getHwoMappingTarget, getHwoMappingGroups } from "./loader.js";
    ...
    for (const entry of entries) {
      if (entry.name) {
        ...
        if (
          ...
          nameNormalized.split(" ").some((part) => normalized.includes(part))
        ) {
...
export function classifyToMappingTarget(
  ...
  const groups = getHwoMappingGroups(systemId);
