/*
<MODULE_CONTRACT>
<purpose>This module extracts structured signals from a raw HTML page, including metadata, contact information, and compliance elements.</purpose>
<non-goals>
  <item>This module does not perform any validation of extracted data.</item>
</non-goals>
</MODULE_CONTRACT>
<CHANGE_SUMMARY>
  <item>Initial creation of the module to extract page signals from HTML.</item>
</CHANGE_SUMMARY>
*/
...
export const extractPageSignals = (html: string | CheerioAPI, baseUrl: string): ExtractResult => {
  ...
  const socialLinks: SocialLink[] = Array.from(socialMap.entries()).map(([platform, url]) => ({
    platform,
    url,
  }));
