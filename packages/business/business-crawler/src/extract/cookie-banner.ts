/*
<MODULE_CONTRACT>
<purpose>This module detects and classifies the quality level of cookie consent banners on web pages, ensuring compliance with privacy standards.</purpose>
<non-goals>
  <item>This module does not handle the display or interaction with cookie banners on web pages.</item>
</non-goals>
</MODULE_CONTRACT>
<CHANGE_SUMMARY>
  <item>Initial implementation of cookie consent banner detection and classification.</item>
</CHANGE_SUMMARY>
*/
...
const REJECT_RE =
  /\b(ablehnen|reject|decline|verweigern|nicht\s*akzeptieren|alle\s*ablehnen|deny|nur\s*notwendige|only\s*necessary|essential\s*only)\b/i;
const ACCEPT_RE =
  /\b(akzeptieren|accept|zustimmen|alle\s*akzeptieren|ich\s*stimme\s*zu|allow|agree|got\s*it|verstanden|ok|einverstanden)\b/i;
const SETTINGS_RE =
  /\b(einstellungen|settings|anpassen|customize|customise|preferences|cookie-einstellungen|details|verwalten|manage|optionen|options)\b/i;
const REVOKE_RE =
  /\b(widerrufen|widerruf|revoke|withdraw|widerrufsrecht|einwilligung\s*widerrufen|consent\s*withdraw)\b/i;
...
const hasButtonMatching = ($: CheerioAPI, scope: ReturnType<CheerioAPI>, re: RegExp): boolean => {
  let found = false;
  scope
    .find('button, a, input[type="button"], input[type="submit"], [role="button"]')
    .each((_, el) => {
