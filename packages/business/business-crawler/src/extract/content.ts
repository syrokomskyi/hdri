/*
<MODULE_CONTRACT>
<purpose>This module extracts various content signals from HTML, such as contact forms, portfolios, maps, team pages, testimonials, certifications, awards, memberships, meister qualifications, and case studies, using keyword matching and HTML structure analysis.</purpose>
<non-goals>
  <item>This module does not perform any HTML fetching or network requests.</item>
</non-goals>
</MODULE_CONTRACT>
<CHANGE_SUMMARY>
  <item>Initial implementation of content extraction functions for various signals.</item>
</CHANGE_SUMMARY>
*/
...
export const extractContactForm = (html: string | CheerioAPI): ContentSignalResult => {
  ...
  $("form").each((_i, form) => {
    if (found) return;
    if ($(form).find('input[type="email"], input[name*="email"], input[name*="mail"]').length > 0)
      found = true;
  });
...
export const extractPortfolio = (html: string | CheerioAPI): ContentSignalResult => {
  ...
  if (
    ...
    bodyText.includes("galerie") ||
    bodyText.includes("referenzen")
  ) {
...
export const extractCertifications = (html: string | CheerioAPI): ContentSignalResult => {
  ...
  const imgAlts = $("img[alt]")
    .map((_i, el) => ($(el).attr("alt") ?? "").toLowerCase())
    .get()
    .join(" ");
...
export const extractAwards = (html: string | CheerioAPI): ContentSignalResult => {
  ...
  const combined = `${bodyText} ${imgAlts}`;
  const found =
    AWARD_TERMS_PLAIN.some((t) => combined.includes(t)) || AWARD_TERMS_WORD.test(combined);
...
export const extractMemberships = (html: string | CheerioAPI): ContentSignalResult => {
  ...
  const bodyText = $("body").text().toLowerCase();
  const found =
    MEMBERSHIP_TERMS_PLAIN.some((t) => bodyText.includes(t)) ||
    MEMBERSHIP_TERMS_WORD.test(bodyText);
  return found ? { present: true, confidence: 65 } : { present: false, confidence: null };
};
