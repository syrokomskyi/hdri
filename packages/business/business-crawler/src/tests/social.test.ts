...
const link = (href: string) => `<html><body><a href="${href}">Follow us</a></body></html>`;
...
describe("extractSocialXing", () => {
  it("detects xing.com link", () => {
    expect(extractSocialXing(link("https://www.xing.com/profile/Max_Mustermann")).present).toBe(
      true,
    );
  });
...
describe("extractSocialWhatsapp", () => {
  ...
  it("detects api.whatsapp.com link", () => {
    expect(extractSocialWhatsapp(link("https://api.whatsapp.com/send?phone=49123")).present).toBe(
      true,
    );
  });
