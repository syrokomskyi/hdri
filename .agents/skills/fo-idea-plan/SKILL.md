...
8. **Stamp implemented** — transition RFC to `implemented` (MANDATORY — do not omit from the plan). This step MUST be the last step in every plan. It includes:
   - Set `status: implemented` in RFC frontmatter.
   - Set `implementedAt: <date>` in RFC frontmatter.
   - Move commands from `commands.proposed` to `commands.added` in RFC frontmatter.
   - Check all acceptance criteria with `[x]` and inline `(evidence: ...)` annotations (V-27).
   - Regenerate command manifest (`command.manifest.generate`) and `COMMANDS.md` (`docs.commands.generate`) if the RFC adds/changes/removes commands.
   - Run `rfc.validate` to confirm the RFC passes all checks.
   - Commit the stamped RFC file separately from implementation commits.
