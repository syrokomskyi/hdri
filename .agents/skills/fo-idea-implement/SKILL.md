...
#### 3.8.1. Post-stamp verification gate

After stamping, re-read the RFC frontmatter and verify ALL of the following. If any check fails, fix it immediately before proceeding to step 3.9:

1. **`status` is `implemented`** — not `accepted`, not `draft`.
2. **`implementedAt` is set** — non-empty date string matching today's date.
3. **`commands.added` includes all new commands** — every command from `commands.proposed` must appear in `commands.added`; `commands.proposed` must be empty.
4. **All acceptance criteria are `[x]` with `(evidence: ...)` annotations** — no unchecked `[ ]` criteria remain (V-26); no checked `[x]` lacks evidence (V-27).
5. **`rfc.validate` passes** — run `ref(forge.yaml bindings.commands.validateRfc) --json` and confirm `"status": "pass"`. If RFC-CMD-02 fails for a cross-repo command (command registered in a different repository), document it as a known false positive in the RFC report (step 3.12) — this is the only acceptable exception.
6. **Command manifest regenerated** — if the RFC adds/changes/removes commands, `docs/command-manifest.generated.yaml` and `docs/COMMANDS.md` must be regenerated and committed.

This gate is MANDATORY and catches the most common implementation failure: forgetting to transition the RFC status after code work is complete.
