...
Scope: applies to `apps/gen/*`. Prefer shared helpers from `@syrokomskyi/pipeline-core`, `@syrokomskyi/pipeline-node`, `@syrokomskyi/pipeline-ai`, and `@syrokomskyi/pipeline-steps` over app-local reimplementations.

## Transformation Order

1. Create app structure (`apps/gen/[name]/`).
...
The app-local `Gogol` base class is the concrete alias used inside apps. It extends `PipelineStep<PipelineContext>` from `@syrokomskyi/pipeline-core/step`. Do not invent names like `PipelineGogol` — the canonical app-local alias is `Gogol` (see [apps/gen/site/run/pipeline/Gogol.ts](apps/gen/site/run/pipeline/Gogol.ts)).
...
Prefer shared helpers from `@syrokomskyi/pipeline-ai` with built-in logging. Pass logger object from context for automatic AI call logging. Never hand-roll provider HTTP calls or reinvent the `AI/ai-<k>/` logging layout.
...
```typescript
import { createAnthropicAiText } from '@syrokomskyi/pipeline-ai';
...
const result = await createAnthropicAiText({
  ...
  messages: [...],
  logger: {
    logCall: ctx.logAiCall,
    writeResponse: ctx.writeAiResponses,
    logStepEvent: ctx.logStepEvent,
  },
});
  userPrompts: ['...'],
  temperature: 0.7,
...
```typescript
import { createOpenAiJson } from '@syrokomskyi/pipeline-ai';
...
- Use shared AI helpers from `@syrokomskyi/pipeline-ai`; do not embed model strings unless the step truly pins a specific version — use `<model>` placeholders in templates.
...
Prefer the shared `WaitHumanStep` from `@syrokomskyi/pipeline-steps` over re-implementing pause/wait logic per app. Use `PipelinePauseError` (from `@syrokomskyi/pipeline-core`) only when a gogol has a truly app-specific pause that cannot be expressed by the shared step.
...
```typescript
import { WaitHumanStep } from '@syrokomskyi/pipeline-steps';
...
- Prefer `WaitHumanStep` from `@syrokomskyi/pipeline-steps`.
...
```typescript
import type { PipelinePhase } from '@syrokomskyi/pipeline-core';
...
Prefer shared declaration materialization helpers from `@syrokomskyi/pipeline-node/declarations` (for example `createDeclaredPhaseOptions`) inside `AppPhase` instead of app-local ad hoc glue.

### Gogol Registry — `simpleFactories` record (preferred for greenfield)

Use this pattern when most factories are zero-argument constructors. Models apps/gen/site — see [apps/gen/site/run/pipeline/gogol-registry.ts](apps/gen/site/run/pipeline/gogol-registry.ts).
...
Use this pattern when most factories need to read parameterized config from the declaration, making the record form noisy. Models apps/gen/inticle.
...
```typescript
import { definePipeline } from '@syrokomskyi/pipeline-core';
import type { PipelineDefinition } from '@syrokomskyi/pipeline-core';
...
- [ ] All AI calls go through `@syrokomskyi/pipeline-ai` helpers and emit `AI/ai-<k>/` logs via the shared logging-aware helper contract.
...
- [ ] Human gates prefer `WaitHumanStep` from `@syrokomskyi/pipeline-steps`; app-specific pauses use `PipelinePauseError`.
