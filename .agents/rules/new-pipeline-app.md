# Creating a new pipeline app in `apps/gen/*`
...
- `apps/gen/site` — primary reference. Greenfield-style assembly, no feature-flag routing, `simpleFactories` record in the gogol registry. **Start here.**
- `apps/gen/inticle` — secondary reference. Adds brief-driven feature flags (`whenFeature: ...`), per-language translation fan-out, multiple phase classes, and a `switch` in the gogol registry. Use it only when the new pipeline genuinely needs conditional phases.
...
- Pick a kebab-case app id (e.g. `letter`, `podcast`, `video-essay`). The pnpm package name will be `@syrokomskyi/<id>`.
...
Create this exact layout under `apps/gen/<id>/`. Missing any of these files means the app is incomplete.
...
2. Root [`tsconfig.json`](../../../tsconfig.json): add `{ "path": "./apps/gen/<id>" }` to `references`.
...
5. `run/pipeline/declaration.ts` (loaders from `@syrokomskyi/pipeline-node/declarations`).
...
```json
{
  ...
  "type": "module",
  "scripts": {
    "build": "pnpm --dir ../../.. exec tsc -p apps/gen/<id>/tsconfig.json --noEmit",
    "lint": "pnpm --dir ../../.. exec eslint apps/gen/<id>/run --ext .ts",
    "typecheck": "pnpm --dir ../../.. exec tsc -p apps/gen/<id>/tsconfig.json --noEmit",
    "test": "pnpm --dir ../../.. exec vitest run --config apps/gen/<id>/vitest.config.ts",
  ...
  "dependencies": {
    "@syrokomskyi/pipeline-ai": "workspace:*",
    "@syrokomskyi/pipeline-core": "workspace:*",
    "@syrokomskyi/pipeline-node": "workspace:*",
    "@syrokomskyi/pipeline-steps": "workspace:*",
    "@types/node": "^26.1.1",
    "dotenv": "^17.4.2",
    "gray-matter": "^4.0.3",
    "handlebars": "^4.7.9",
...
TypeScript resolution is provided by root `tsconfig.base.json` custom conditions and package `exports`. Do not add app-local `paths` mappings for `@syrokomskyi/*` packages; they duplicate package exports and mask source-condition drift. Copy from [`apps/gen/site/tsconfig.json`](../../apps/gen/site/tsconfig.json) and adjust references for the packages the app imports.
...
```json
{
  ...
  "references": [
    { "path": "../../../packages/pipeline/pipeline-steps" },
    { "path": "../../../packages/pipeline/pipeline-node" },
    { "path": "../../../packages/pipeline/pipeline-ai" },
    { "path": "../../../packages/pipeline/pipeline-core" }
  ]
}
```

After writing this file, add `{ "path": "./apps/gen/<id>" }` to the root [`tsconfig.json`](../../../tsconfig.json) `references` array.

## 5. Runtime bootstrap layer

Derive from `apps/gen/site` verbatim, adjusting only:
...
- `run/app/create-clients.ts` — instantiate only the providers the app actually uses; read keys via `getRequiredEnv` from `@syrokomskyi/pipeline-node/env` (never `process.env.XYZ` directly).
...
Use `createPipelineDeclarationLoaders` from `@syrokomskyi/pipeline-node/declarations`. Never reimplement markdown frontmatter parsing or member-resolution caching — those belong in `packages/pipeline/pipeline-node`. Re-export only the helpers the app needs.
...
- `decisionType: human_confirms` or `client_chooses` gogols must rely on shared human-gate abstractions from `@syrokomskyi/pipeline-steps` (`WaitHumanStep`, etc.), not bespoke pause logic.
...
- Extend the app-local `Gogol` base (which itself extends shared `PipelineStep<PipelineContext>` from `@syrokomskyi/pipeline-core/step`). Do **not** extend `PipelineStep` directly in an app file.
- Declare `artifacts` with typed validators. Prefer shared validators (e.g. `validateMarkdown`, `validateJsonWithArray`) over hand-rolled ones. **All structured artifacts must be YAML (`.yaml`), not JSON** — see root `AGENTS.md` § YAML artifact convention.
...
- `run(ctx)` must:
  ...
  - For idempotent template-driven artifacts, use `writeGogolTemplateArtifactsIfMissing` from the app `utils.ts` (which wraps `@syrokomskyi/pipeline-node/templates`).
...
- **`simpleFactories` record** (preferred for greenfield): map `factory` string → `() => new Gogol()`. See [`apps/gen/site/run/pipeline/gogol-registry.ts`](../../apps/gen/site/run/pipeline/gogol-registry.ts).
- **`switch` on `declaration.factory`**: required only when factories need parameterized config from the declaration (`conceptIndex`, `circleNumber`, `requiredOutputFiles`, …). See [`apps/gen/inticle/run/pipeline/gogol-registry.ts`](../../apps/gen/inticle/run/pipeline/gogol-registry.ts).
...
pnpm turbo run typecheck --filter=@syrokomskyi/<id>
pnpm turbo run lint --filter=@syrokomskyi/<id>
pnpm turbo run start --filter=@syrokomskyi/<id>
```
...
- Calling `JSON.parse` directly on LLM output. Always go through `createOpenAiJson` or `parseAiJson` from `@syrokomskyi/pipeline-ai`.
- Hand-rolling AI logging (writing prompts/responses manually to `ai-<k>/`). Always pass logger object to AI provider functions from `@syrokomskyi/pipeline-ai`.
...
- Using `process.env.XYZ` directly. Always go through `getRequiredEnv` from `@syrokomskyi/pipeline-node/env`.
