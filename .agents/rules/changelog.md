...
pnpm --filter @warpgogol/changelog-live exec tsx -C @syrokomskyi/source src/cli.ts -c changelog.config.yaml
```
...
The `@warpgogol/changelog-live` devDependency and `changelog` / `changelog:init` scripts must be present in the workspace's `package.json`. Most workspaces already have them — copy from an existing package if needed.
