...
4. **Generate declarations first, code second**:
   ...
   - Prefer shared frontmatter/declaration helpers from `packages/pipeline/pipeline-node` instead of app-local markdown parsing code
...
5. **Reuse shared packages aggressively**:
   ...
   - Prefer `packages/pipeline/pipeline-steps` for reusable human gates and pause-like steps
   - Prefer `createOpenAiJson(...)` when migrating OpenAI text calls that expect structured JSON
   - Pass logger object from context to AI provider functions for built-in logging instead of hand-written AI logging wrappers
