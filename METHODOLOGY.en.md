...
The implemented methodology is fully encoded in [`codebook.yaml`](apps/hdri/observatory/.input/codebook.yaml) (scoring rules) and [`ontology.yaml`](apps/hdri/observatory/.input/ontology.yaml) (signal catalog). This document is the human-readable summary.
...
- **Anlage A and Anlage B of the Handwerksordnung (HWO)** — the statutory lists of licensed and unlicensed crafts in Germany. Source: [`Gesetze im Internet`](https://www.gesetze-im-internet.de/hwo/anlage_a.html) and [`Anlage B`](https://www.gesetze-im-internet.de/hwo/anlage_b.html). These data are versioned as machine-readable JSON in [`packages/business/business-core/src/gewerk/data/hwo-master.json`](packages/business/business-core/src/gewerk/data/hwo-master.json).

- **Destatis business groups I–VII** — the Federal Statistical Office (Destatis) maps HWO crafts into seven business groups. The mapping is documented in [`packages/business/business-core/src/gewerk/data/destatis-mapping.json`](packages/business/business-core/src/gewerk/data/destatis-mapping.json) and is based on the publication _„Gewerbegruppen der Handwerksstatistiken nach Handwerksordnung Stand 2021"_.
...
A **signal** is an observable property of a website captured by an automated extractor (crawler, DOM analyser, Lighthouse, axe). The complete list of signals lives in [`ontology.yaml`](apps/hdri/observatory/.input/ontology.yaml).
...
The implemented rules are in [`codebook.yaml`](apps/hdri/observatory/.input/codebook.yaml) under `dimensions`.
...
If a domain has multiple crawled pages (e.g. homepage + imprint), the **maximum operator** is applied across all pages: if the imprint exists on _any_ crawled page, the domain is counted as positive.
...
### 6.4 Cross-section comparison vs. panel (longitudinal)

The published descriptive index is a **repeated cross-section**: each quarter aggregates whichever sample was drawn that quarter. If the sampling frame changes substantially between two quarters (e.g. a tripling of the sample), a naive quarter-over-quarter delta conflates real change with **change in composition** (Simpson's paradox): the overall index can fall even though every segment improved.
...
Scores from different codebook or ontology versions are **not comparable**. If two compared quarters differ in `codebook_version` or `ontology_version`, the delta is **hard-suppressed** (`codebook_version_changed` / `ontology_version_changed`). The authoritative scoring version is read from the `scores` rows (not from run-level metadata).
