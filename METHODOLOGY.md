...
Die implementierte Methodik ist vollständig in [`codebook.yaml`](apps/hdri/observatory/.input/codebook.yaml) (Bewertungsregeln) und [`ontology.yaml`](apps/hdri/observatory/.input/ontology.yaml) (Signalkatalog) kodiert. Dieses Dokument stellt die menschenlesbare Zusammenfassung dar.
...
- **Deutsches und EU-Recht** — § 5 DDG, Art. 13/14 DSGVO, Barrierefreiheitsstärkungsgesetz (BFSG)
...
- **Anlage A und Anlage B der Handwerksordnung (HWO)** — die gesetzlichen Listen der zulassungspflichtigen und zulassungsfreien Handwerke in Deutschland. Quelle: [`Gesetze im Internet`](https://www.gesetze-im-internet.de/hwo/anlage_a.html) und [`Anlage B`](https://www.gesetze-im-internet.de/hwo/anlage_b.html). Diese Daten sind als maschinenlesbare JSON-Dateien in [`packages/business/business-core/src/gewerk/data/hwo-master.json`](packages/business/business-core/src/gewerk/data/hwo-master.json) versioniert.

- **Destatis-Gewerbegruppen I–VII** — das Statistische Bundesamt (Destatis) ordnet die HWO-Handwerke in sieben Gewerbegruppen ein. Die Zuordnung ist in [`packages/business/business-core/src/gewerk/data/destatis-mapping.json`](packages/business/business-core/src/gewerk/data/destatis-mapping.json) dokumentiert und beruht auf der Publikation _„Gewerbegruppen der Handwerksstatistiken nach Handwerksordnung Stand 2021"_.
...
Ein **Signal** ist eine beobachtbare Eigenschaft einer Website, die von einem automatisierten Extraktor (Crawler, DOM-Analysator, Lighthouse, axe) erfasst wird. Die vollständige Liste aller Signale lebt in [`ontology.yaml`](apps/hdri/observatory/.input/ontology.yaml).

| Signal-Kategorie | Beispiele | Quelle |
| --- | --- | --- |
| `legal.*` | Impressum, Datenschutzerklärung, AGB, BFSG-Erklärung | Homepage-Crawl + Regel-Matching (§ 5 DDG, Art. 13/14 DSGVO, BFSG) |
...
| --- | --- | --- |
| **Rechtskonformität** (`legal_compliance`) | 28 % | Höchstes Risiko bei Abwesenheit (§ 5 DDG, DSGVO, BFSG). |
...
Die implementierten Regeln finden sich in [`codebook.yaml`](apps/hdri/observatory/.input/codebook.yaml) unter `dimensions`.
...
Falls eine Domain mehrere gecrawlte Seiten hat (z. B. Startseite + Impressum), wird der **Maximum-Operator** über alle Seiten angewendet: wenn das Impressum auf _irgendeiner_ gecrawlten Seite vorhanden ist, gilt die Domain als positiv.
...
### 6.4 Querschnittsvergleich vs. Panel (Längsschnitt)

Der veröffentlichte deskriptive Index ist ein **wiederholter Querschnitt**: Jedes Quartal wird die jeweils gezogene Stichprobe aggregiert. Ändert sich der Stichprobenrahmen zwischen zwei Quartalen erheblich (z. B. eine Verdreifachung der Stichprobe), vermischt ein naiver Quartalsdelta echte Veränderung mit **Veränderung der Zusammensetzung** (Simpson-Paradoxon): Der Gesamtindex kann sinken, obwohl jedes Segment sich verbessert hat.
...
Scores aus unterschiedlichen Codebook- oder Ontologie-Versionen sind **nicht vergleichbar**. Unterscheiden sich zwei verglichene Quartale in `codebook_version` oder `ontology_version`, wird der Delta **hart unterdrückt** (`codebook_version_changed` / `ontology_version_changed`). Die maßgebliche Scoring-Version wird aus den `scores`-Zeilen gelesen (nicht aus run-bezogenen Metadaten).
...
| § 5 DDG (Digitale-Dienste-Gesetz) | Rechtsgrundlage für Impressumspflicht |
